...
import { PersonModal } from './dialog/modal'
...
export const Person = () => {
  ...
  const getUserList = () => {
    client(`person/list?${qs.stringify(pagination)}`, { method: "POST" }).then(res => {
      setData(res.data)
  ...
  return (
    <div>
      ...
      <Main>
        ...
        {isShow ? <PersonModal formData={formData} formType={formType} isShow={isShow} setIsShow={setIsShow} getUserList={getUserList} /> : ''}
      </Main>
    </div>
  )
}
