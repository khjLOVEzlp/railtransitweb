// 广州线路
export const guangzhouSubwayList = [
  {
    name: "地铁1号线",
    tooltip: {
      formatter: "{b}:地铁1号线<br />",
    },
    symbolSize: 0.1,
    value: [502, -1204],
    fixed: true,
    category: 1,
    label: {
      color: "#F9E103",
      position: "bottom",
      fontSize: 11,
      fontWeight: 700,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "西塱",
        tooltip: { formatter: "{b}" },
        value: [562, -1470],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
      },
      {
        name: "坑口",
        tooltip: { formatter: "" },
        value: [562, -1398],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
      },
      {
        name: "花地湾",
        value: [562, -1326],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "芳村",
        value: [562, -1254],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "黄沙",
        value: [494, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "长寿路",
        value: [494, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "陈家祠",
        value: [600, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "西门口",
        value: [668, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "公园前",
        value: [734, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "农讲所",
        value: [819, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "烈士陵园",
        value: [882, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "东山口",
        value: [948, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "杨箕",
        value: [995, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "体育西路",
        value: [1077, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "体育中心",
        value: [1206, -984],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "广州东站",
        value: [1077, -938],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#F9E103", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁2号线",
    tooltip: { formatter: "{b}:地铁2号线<br />" },
    symbolSize: 0.1,
    value: [844, -835],
    fixed: true,
    category: 1,
    label: {
      color: "#4f90cb",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "嘉禾望岗",
        value: [898, -750],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "黄边",
        value: [866, -783],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "江夏",
        value: [840, -809],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "萧岗",
        value: [814, -835],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "白云文化广场",
        value: [788, -861],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "白云公园",
        value: [762, -887],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "飞翔公园",
        value: [734, -913],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "三元里",
        value: [734, -947],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "广州火车站",
        value: [734, -982],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "越秀公园",
        value: [734, -1020],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "纪念堂",
        value: [734, -1059],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "公园前",
        value: [734, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "海珠广场",
        value: [734, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "市二宫",
        value: [734, -1205],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "江南西",
        value: [734, -1267],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "昌岗",
        value: [734, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "江泰路",
        value: [734, -1395],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "东晓南",
        value: [734, -1460],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "南洲",
        value: [734, -1525],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "洛溪",
        value: [734, -1590],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "南浦",
        value: [734, -1655],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "会江",
        value: [734, -1720],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "石壁",
        value: [734, -1785],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "广州南站",
        value: [734, -1863],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#4f90cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁3号线",
    tooltip: { formatter: "{b}:地铁3号线<br />" },
    symbolSize: 0.1,
    value: [1276, -1018],
    fixed: true,
    category: 1,
    label: {
      color: "#EA6632",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "天河客运站",
        value: [1378, -893],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "五山",
        value: [1380, -994],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "华师",
        value: [1332, -1036],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "岗顶",
        value: [1276, -1078],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "石牌桥",
        value: [1213, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "体育西路",
        value: [1077, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "珠江新城",
        value: [1077, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "广州塔",
        value: [1077, -1268],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "客村",
        value: [1077, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大塘",
        value: [1077, -1410],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "沥滘",
        value: [1077, -1474],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "厦滘",
        value: [1077, -1525],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大石",
        value: [1077, -1579],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "汉溪长隆",
        value: [1077, -1631],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "市桥",
        value: [1126, -1661],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "番禺广场",
        value: [1198, -1661],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁3号线北延段",
    tooltip: { formatter: "{b}:地铁3号线北延段<br />" },
    symbolSize: 0.1,
    value: [1000, -803],
    fixed: true,
    category: 1,
    label: {
      color: "#EA6632",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "体育西路",
        value: [1077, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "林和西",
        value: [1077, -1001],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "广州东站",
        value: [1077, -938],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "燕塘",
        value: [1077, -893],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "梅花园",
        value: [1050, -870],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "京溪南方医院",
        value: [1020, -846],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "同和",
        value: [990, -823],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "永泰",
        value: [960, -800],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "白云大道北",
        value: [931, -776],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "嘉禾望岗",
        value: [898, -750],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "龙归",
        value: [898, -698],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "人和",
        value: [898, -653],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "高增",
        value: [898, -615],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "机场南(1号航站楼)",
        value: [898, -563],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "机场北(2号航站楼)",
        value: [898, -498],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#EA6632", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁4号线",
    tooltip: { formatter: "{b}:地铁4号线<br />" },
    symbolSize: 0.1,
    value: [1888, -1965],
    fixed: true,
    category: 1,
    label: {
      color: "#009900",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#009900", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "黄村",
        value: [1438, -1115],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "车陂",
        value: [1399, -1161],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "车陂南",
        value: [1358, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "万胜围",
        value: [1358, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "官洲",
        value: [1358, -1392],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大学城北",
        value: [1358, -1452],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大学城南",
        value: [1358, -1511],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "新造",
        value: [1358, -1570],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "石碁",
        value: [1358, -1630],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "海傍",
        value: [1358, -1689],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "低涌",
        value: [1358, -1748],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "东涌",
        value: [1358, -1807],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "庆盛",
        value: [1400, -1832],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "黄阁汽车城",
        value: [1450, -1862],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "黄阁",
        value: [1533, -1862],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "蕉门",
        value: [1601, -1862],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "金洲",
        value: [1671, -1862],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "飞沙角",
        value: [1671, -1941],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "广隆",
        value: [1671, -2034],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大涌",
        value: [1755, -2076],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "塘坑",
        value: [1900, -2076],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "南横",
        value: [2047, -1961],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "南沙客运港",
        value: [2047, -1862],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#009900", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁5号线",
    tooltip: { formatter: "{b}:地铁5号线<br />" },
    symbolSize: 0.1,
    value: [362, -1138],
    fixed: true,
    category: 1,
    label: {
      color: "#FF0000",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "滘口",
        value: [352, -1118],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "坦尾",
        value: [403, -1059],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "中山八",
        value: [561, -1059],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "西场",
        value: [600, -1020],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "西村",
        value: [638, -982],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "广州火车站",
        value: [734, -982],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "小北",
        value: [820, -982],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "淘金",
        value: [904, -982],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "区庄",
        value: [948, -982],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "动物园",
        value: [995, -1035],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "杨箕",
        value: [995, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "五羊邨",
        value: [995, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "珠江新城",
        value: [1077, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "猎德",
        value: [1133, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "潭村",
        value: [1190, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "员村",
        value: [1246, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "科韵路",
        value: [1302, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "车陂南",
        value: [1358, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "东圃",
        value: [1391, -1237],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "三溪",
        value: [1427, -1269],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "鱼珠",
        value: [1467, -1309],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大沙地",
        value: [1548, -1309],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大沙东",
        value: [1624, -1309],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "文冲",
        value: [1704, -1309],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#FF0000", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁6号线",
    tooltip: {
      formatter: "{b}:地铁6号线<br />",
    },
    symbolSize: 0.1,
    value: [2003, -804],
    fixed: true,
    category: 1,
    label: {
      color: "#8B1F5C",
      position: "bottom",
      fontSize: 12,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: {
          color: "#fff",
          borderColor: "#8B1F5C",
          borderWidth: 3,
        },
      },
    },

    stations: [
      {
        name: "香雪",
        value: [2023, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "萝岗",
        value: [1961, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "苏元",
        value: [1903, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "暹岗",
        value: [1826, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "金峰",
        value: [1766, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "黄陂",
        value: [1687, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "高塘石",
        value: [1614, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "柯木塱",
        value: [1552, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "龙洞",
        value: [1487, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "植物园",
        value: [1405, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "长湴",
        value: [1378, -836],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "天河客运站",
        value: [1378, -893],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "燕塘",
        value: [1077, -893],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "天平架",
        value: [1038, -893],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "沙河顶",
        value: [1008, -918],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "黄花岗",
        value: [1008, -942],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "区庄",
        value: [948, -982],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "东山口",
        value: [948, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "东湖",
        value: [948, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "团一大广场",
        value: [879, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "北京路",
        value: [808, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "海珠广场",
        value: [734, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "一德路",
        value: [667, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "文化公园",
        value: [573, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "黄沙",
        value: [494, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "如意坊",
        value: [443, -1102],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "坦尾",
        value: [403, -1059],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "河沙",
        value: [403, -982],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "沙贝",
        value: [403, -920],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "横沙",
        value: [403, -857],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "浔峰岗",
        value: [403, -795],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#8B1F5C", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁7号线",
    tooltip: { formatter: "{b}:地铁7号线<br />" },
    symbolSize: 0.1,
    value: [802, -1813],
    fixed: true,
    category: 1,
    label: {
      color: "#3FA37E",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "广州南站",
        value: [742, -1863],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "石壁",
        value: [742, -1785],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "谢村",
        value: [832, -1746],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "钟村",
        value: [967, -1682],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "汉溪长隆",
        value: [1077, -1631],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "南村万博",
        value: [1135, -1606],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "员岗",
        value: [1223, -1568],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "板桥",
        value: [1300, -1537],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大学城南",
        value: [1358, -1511],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#3FA37E", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁8号线",
    tooltip: { formatter: "{b}:地铁8号线<br />" },
    symbolSize: 0.1,
    value: [583, -568],
    fixed: true,
    category: 1,
    label: {
      color: "#00A1CC",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "滘心",
        value: [533, -618],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "亭岗",
        value: [533, -670],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "石井",
        value: [585, -709],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "小坪",
        value: [638, -748],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "石潭",
        value: [638, -787],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "聚龙",
        value: [638, -826],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "上步",
        value: [638, -865],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "同德",
        value: [638, -904],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "鹅掌坦",
        value: [638, -943],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "西村",
        value: [638, -982],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "陈家祠",
        value: [600, -1117],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "华林寺",
        value: [586, -1142],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "文化公园",
        value: [573, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "同福西",
        value: [604, -1216],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "凤凰新村",
        value: [586, -1265],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "沙园",
        value: [638, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "宝岗大道",
        value: [690, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "昌岗",
        value: [734, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "晓港",
        value: [796, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "中大",
        value: [875, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "鹭江",
        value: [953, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "客村",
        value: [1077, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "赤岗",
        value: [1167, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "磨碟沙",
        value: [1217, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "新港东",
        value: [1267, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "琶洲",
        value: [1316, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "万胜围",
        value: [1358, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00A1CC", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁9号线",
    tooltip: { formatter: "{b}:地铁9号线<br />" },
    symbolSize: 0.1,
    value: [140, -565],
    fixed: true,
    category: 1,
    label: {
      color: "#99c857",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#99c857", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "高增",
        value: [890, -615],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "清塘",
        value: [833, -550],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "清布",
        value: [768, -550],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "莲塘",
        value: [703, -550],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "马鞍山公园",
        value: [651, -485],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "花都广场",
        value: [547, -485],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "花果山公园",
        value: [526, -516],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "花城路",
        value: [401, -550],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "广州北站",
        value: [297, -550],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "花都汽车城",
        value: [193, -550],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "飞鹅岭",
        value: [89, -550],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#99c857", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁13号线",
    tooltip: { formatter: "{b}:地铁13号线<br />" },
    symbolSize: 0.1,
    value: [2107, -1459],
    fixed: true,
    category: 1,
    label: {
      color: "#a7b942",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "鱼珠",
        value: [1467, -1309],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "裕丰围",
        value: [1467, -1355],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "双岗",
        value: [1520, -1413],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "南海神庙",
        value: [1594, -1413],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "夏园",
        value: [1672, -1413],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "南岗",
        value: [1751, -1413],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "沙村",
        value: [1826, -1413],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "白江",
        value: [1896, -1413],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "新塘",
        value: [1965, -1413],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "官湖",
        value: [2042, -1413],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "新沙",
        value: [2129, -1413],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#a7b942", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁14号线",
    tooltip: { formatter: "{b}:地铁14号线<br />" },
    symbolSize: 0.1,
    value: [1458, -100],
    fixed: true,
    category: 1,
    label: {
      color: "#721920",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#721920", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "嘉禾望岗",
        value: [898, -750],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "白云东平",
        value: [950, -698],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "夏良",
        value: [1002, -646],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "太和",
        value: [1054, -594],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "竹料",
        value: [1106, -542],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "钟落潭",
        value: [1158, -490],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "马沥",
        value: [1210, -438],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "新和",
        value: [1275, -373],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "太平",
        value: [1327, -321],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "神岗",
        value: [1379, -269],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "赤草",
        value: [1431, -217],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "从化客运站",
        value: [1483, -165],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "东风",
        value: [1535, -113],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁21号线",
    tooltip: {
      formatter: "{b}:地铁21号线<br />",
    },
    symbolSize: 0.1,
    value: [2196, -358],
    fixed: true,
    category: 1,
    label: {
      color: "#203082",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#203082", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "员村",
        value: [1246, -1208],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "天河公园",
        value: [1289, -1161],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "棠东",
        value: [1380, -1115],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "黄村",
        value: [1438, -1115],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大观南路",
        value: [1438, -1076],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "天河智慧城",
        value: [1554, -990],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "神舟路",
        value: [1670, -931],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "科学城",
        value: [1787, -871],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "苏元",
        value: [1903, -784],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "水西",
        value: [1903, -732],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "长平",
        value: [1903, -687],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "金坑",
        value: [1903, -635],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "镇龙西",
        value: [1903, -583],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "镇龙",
        value: [1903, -544],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "中新",
        value: [1903, -492],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "坑贝",
        value: [1903, -427],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "凤岗",
        value: [1968, -394],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "朱村",
        value: [2033, -362],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "山田",
        value: [2098, -329],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "钟岗",
        value: [2163, -298],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "增城广场",
        value: [2228, -266],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#203082", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "广佛线",
    tooltip: { formatter: "{b}:广佛线<br />" },
    symbolSize: 0.1,
    value: [447, -1934],
    fixed: true,
    category: 1,
    label: {
      color: "#c9e887",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "沥滘",
        value: [1077, -1474],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "南洲",
        value: [734, -1525],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "石溪",
        value: [712, -1492],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "燕岗",
        value: [712, -1427],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "沙园",
        value: [638, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "沙涌",
        value: [602, -1332],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "鹤洞",
        value: [602, -1401],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "西塱",
        value: [562, -1470],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "菊树",
        value: [523, -1470],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "龙溪",
        value: [484, -1470],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "金融高新区",
        value: [445, -1470],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "千灯湖",
        value: [445, -1538],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "礌岗",
        value: [445, -1603],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "南桂路",
        value: [445, -1668],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "桂城",
        value: [406, -1668],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "朝安",
        value: [367, -1668],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "普君北路",
        value: [315, -1668],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "祖庙",
        value: [263, -1668],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "同济路",
        value: [263, -1733],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "季华园",
        value: [263, -1785],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "魁奇路",
        value: [263, -1850],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "澜石",
        value: [263, -1915],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "世纪莲",
        value: [287, -1967],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "东平",
        value: [339, -1967],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "新城东",
        value: [391, -1967],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#c9e887", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "apm线",
    tooltip: { formatter: "{b}:apm线<br />" },
    symbolSize: 0.1,
    value: [1127, -951],
    fixed: true,
    category: 1,
    label: {
      color: "#00a1cb",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "林和西",
        value: [1077, -1001],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "体育中心南",
        value: [1100, -1026],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "天河南",
        value: [1100, -1055],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "黄埔大道",
        value: [1100, -1082],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "妇儿中心",
        value: [1100, -1106],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "花城大道",
        value: [1100, -1139],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "大剧院",
        value: [1100, -1166],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "海心沙",
        value: [1100, -1193],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "广州塔",
        value: [1077, -1268],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#00a1cb", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
  {
    name: "地铁14号线支线(知识城线)",
    tooltip: { formatter: "{b}:地铁14号线支线(知识城线)<br />" },
    symbolSize: 0.1,
    value: [1625, -523],
    fixed: true,
    category: 1,
    label: {
      color: "#721920",
      position: "bottom",
      fontSize: 11,
      fontWeight: 1000,
    },
    itemStyle: {
      normal: {
        normal: { color: "#fff", borderColor: "#721920", borderWidth: 3 },
      },
    },
    stations: [
      {
        name: "新和",
        value: [1275, -373],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "红卫",
        value: [1370, -334],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "新南",
        value: [1436, -352],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "枫下",
        value: [1502, -383],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "知识城",
        value: [1564, -411],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "何棠下",
        value: [1631, -443],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "旺村",
        value: [1698, -475],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "汤村",
        value: [1767, -508],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "镇龙北",
        value: [1838, -541],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
      {
        name: "镇龙",
        value: [1903, -544],
        fixed: true,
        symbol: "circle",
        symbolSize: [10, 10],
        label: { color: "#000", position: "top", fontSize: 11 },
        category: 1,
        itemStyle: {
          normal: { color: "#fff", borderColor: "#721920", borderWidth: 2 },
        },
        tooltip: { formatter: "" },
      },
    ],
  },
]

// 上海线路
export const shanghaiSubwayList = [
  {
    "name": "地铁1号线",
    "tooltip": {
      "formatter": "{b}:地铁1号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1859,
      -489
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#e8787a",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#e8787a",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "富锦路",
        "value": [
          1837,
          -195
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "友谊西路",
        "value": [
          1837,
          -241
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宝安公路",
        "value": [
          1837,
          -287
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "共富新村",
        "value": [
          1837,
          -333
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "呼兰路",
        "value": [
          1837,
          -379
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "通河新村",
        "value": [
          1837,
          -425
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "共康路",
        "value": [
          1837,
          -471
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "彭浦新村",
        "value": [
          1837,
          -517
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "汶水路",
        "value": [
          1837,
          -563
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海马戏城",
        "value": [
          1837,
          -608
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "延长路",
        "value": [
          1837,
          -654
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中山北路",
        "value": [
          1837,
          -700
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海火车站",
        "value": [
          1837,
          -764
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "汉中路",
        "value": [
          1837,
          -818
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "新闸路",
        "value": [
          1896,
          -879
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "人民广场",
        "value": [
          1943,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "一大会址·黄陂南路",
        "value": [
          1890,
          -1021
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陕西南路",
        "value": [
          1810,
          -1021
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "常熟路",
        "value": [
          1763,
          -1021
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "衡山路",
        "value": [
          1710,
          -1059
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "徐家汇",
        "value": [
          1666,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海体育馆",
        "value": [
          1608,
          -1161
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "漕宝路",
        "value": [
          1509,
          -1258
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海南站",
        "value": [
          1509,
          -1402
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "锦江乐园",
        "value": [
          1469,
          -1443
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "莲花路",
        "value": [
          1427,
          -1485
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "外环路",
        "value": [
          1385,
          -1527
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "莘庄",
        "value": [
          1342,
          -1572
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#e8787a",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁2号线",
    "tooltip": {
      "formatter": "{b}:地铁2号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1260,
      -906
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#009900",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#009900",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "徐泾东",
        "value": [
          975,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹桥火车站",
        "value": [
          1053,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹桥2号航站楼",
        "value": [
          1157,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "淞虹路",
        "value": [
          1246,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "北新泾",
        "value": [
          1322,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "威宁路",
        "value": [
          1398,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "娄山关路",
        "value": [
          1475,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中山公园",
        "value": [
          1546,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江苏路",
        "value": [
          1668,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "静安寺",
        "value": [
          1760,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南京西路",
        "value": [
          1810,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "人民广场",
        "value": [
          1943,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南京东路",
        "value": [
          2051,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陆家嘴",
        "value": [
          2123,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东昌路",
        "value": [
          2172,
          -976
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "世纪大道",
        "value": [
          2222,
          -1022
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海科技馆",
        "value": [
          2292,
          -1097
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "世纪公园",
        "value": [
          2292,
          -1173
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙阳路",
        "value": [
          2449,
          -1229
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "张江高科",
        "value": [
          2513,
          -1229
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金科路",
        "value": [
          2578,
          -1229
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "广兰路",
        "value": [
          2643,
          -1229
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "唐镇",
        "value": [
          2708,
          -1229
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "创新中路",
        "value": [
          2743,
          -1261
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "华夏东路",
        "value": [
          2743,
          -1306
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "川沙",
        "value": [
          2743,
          -1352
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "凌空路",
        "value": [
          2743,
          -1397
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "远东大道",
        "value": [
          2743,
          -1443
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "海天三路",
        "value": [
          2743,
          -1488
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "浦东国际机场",
        "value": [
          2743,
          -1534
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#009900",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁3号线",
    "tooltip": {
      "formatter": "{b}:地铁3号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      2099,
      -347
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#F9E103",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#F9E103",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "江杨北路",
        "value": [
          2086,
          -86
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "铁力路",
        "value": [
          2086,
          -128
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "友谊路",
        "value": [
          2086,
          -169
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宝杨路",
        "value": [
          2086,
          -210
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "水产路",
        "value": [
          2086,
          -251
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "淞滨路",
        "value": [
          2086,
          -291
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "张华浜",
        "value": [
          2086,
          -332
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "淞发路",
        "value": [
          2086,
          -373
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "长江南路",
        "value": [
          2086,
          -414
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "殷高西路",
        "value": [
          2086,
          -455
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江湾镇",
        "value": [
          2086,
          -495
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大柏树",
        "value": [
          2086,
          -536
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "赤峰路",
        "value": [
          2086,
          -578
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹口足球场",
        "value": [
          2086,
          -623
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东宝兴路",
        "value": [
          2017,
          -690
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宝山路",
        "value": [
          1892,
          -764
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海火车站",
        "value": [
          1837,
          -764
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中潭路",
        "value": [
          1723,
          -764
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "镇坪路",
        "value": [
          1603,
          -764
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "曹杨路",
        "value": [
          1546,
          -802
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金沙江路",
        "value": [
          1546,
          -850
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中山公园",
        "value": [
          1546,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "延安西路",
        "value": [
          1546,
          -982
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹桥路",
        "value": [
          1546,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宜山路",
        "value": [
          1546,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "漕溪路",
        "value": [
          1615,
          -1239
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙漕路",
        "value": [
          1615,
          -1295
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "石龙路",
        "value": [
          1568,
          -1344
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海南站",
        "value": [
          1509,
          -1402
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#F9E103",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁4号线",
    "tooltip": {
      "formatter": "{b}:地铁4号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      2165,
      -891
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#660066",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#660066",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "宝山路",
        "value": [
          1892,
          -772
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海火车站",
        "value": [
          1837,
          -772
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中潭路",
        "value": [
          1723,
          -772
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "镇坪路",
        "value": [
          1603,
          -772
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "曹杨路",
        "value": [
          1554,
          -802
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金沙江路",
        "value": [
          1554,
          -850
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中山公园",
        "value": [
          1554,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "延安西路",
        "value": [
          1554,
          -982
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹桥路",
        "value": [
          1554,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宜山路",
        "value": [
          1554,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海体育馆",
        "value": [
          1608,
          -1161
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海体育场",
        "value": [
          1659,
          -1214
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东安路",
        "value": [
          1763,
          -1240
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大木桥路",
        "value": [
          1867,
          -1240
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "鲁班路",
        "value": [
          1927,
          -1240
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "西藏南路",
        "value": [
          1989,
          -1240
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南浦大桥",
        "value": [
          2051,
          -1240
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "塘桥",
        "value": [
          2120,
          -1240
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "蓝村路",
        "value": [
          2167,
          -1193
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "浦电路(4号线)",
        "value": [
          2222,
          -1094
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "世纪大道",
        "value": [
          2222,
          -1022
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "浦东大道",
        "value": [
          2222,
          -907
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "杨树浦路",
        "value": [
          2222,
          -846
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大连路",
        "value": [
          2222,
          -808
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "临平路",
        "value": [
          2164,
          -736
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "海伦路",
        "value": [
          2120,
          -738
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宝山路",
        "value": [
          1892,
          -772
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#660066",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁5号线支线(东川路-闵行开发区)",
    "tooltip": {
      "formatter": "{b}:地铁5号线支线(东川路-闵行开发区)<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1093,
      -2004
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#CC00CC",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#CC00CC",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "东川路",
        "value": [
          1342,
          -1888
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金平路",
        "value": [
          1293,
          -1958
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "华宁路",
        "value": [
          1233,
          -2017
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "文井路",
        "value": [
          1114,
          -2056
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "闵行开发区",
        "value": [
          1017,
          -2056
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁5号线",
    "tooltip": {
      "formatter": "{b}:地铁5号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1210,
      -2264
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#CC00CC",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#CC00CC",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "莘庄",
        "value": [
          1342,
          -1572
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "春申路",
        "value": [
          1342,
          -1624
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "银都路",
        "value": [
          1342,
          -1672
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "颛桥",
        "value": [
          1342,
          -1726
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "北桥",
        "value": [
          1342,
          -1779
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "剑川路",
        "value": [
          1342,
          -1832
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东川路",
        "value": [
          1342,
          -1888
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江川路",
        "value": [
          1342,
          -1966
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "西渡",
        "value": [
          1342,
          -2057
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "萧塘",
        "value": [
          1342,
          -2135
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "奉浦大道",
        "value": [
          1342,
          -2213
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "环城东路",
        "value": [
          1394,
          -2304
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "望园路",
        "value": [
          1459,
          -2304
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金海湖",
        "value": [
          1537,
          -2382
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "奉贤新城",
        "value": [
          1537,
          -2460
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#CC00CC",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁6号线",
    "tooltip": {
      "formatter": "{b}:地铁6号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      2556,
      -475
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#FF3265",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#FF3265",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "港城路",
        "value": [
          2621,
          -219
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "外高桥保税区北",
        "value": [
          2621,
          -272
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "航津路",
        "value": [
          2621,
          -332
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "外高桥保税区南",
        "value": [
          2621,
          -390
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "洲海路",
        "value": [
          2621,
          -450
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "五洲大道",
        "value": [
          2621,
          -509
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东靖路",
        "value": [
          2621,
          -577
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "巨峰路",
        "value": [
          2608,
          -637
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "五莲路",
        "value": [
          2565,
          -680
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "博兴路",
        "value": [
          2524,
          -721
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金桥路",
        "value": [
          2482,
          -763
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "云山路",
        "value": [
          2441,
          -804
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "德平路",
        "value": [
          2399,
          -846
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "北洋泾路",
        "value": [
          2358,
          -887
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "民生路",
        "value": [
          2316,
          -929
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "源深体育中心",
        "value": [
          2277,
          -967
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "世纪大道",
        "value": [
          2222,
          -1022
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "浦电路(6号线)",
        "value": [
          2167,
          -1094
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "蓝村路",
        "value": [
          2167,
          -1193
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海儿童医学中心",
        "value": [
          2167,
          -1258
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "临沂新村",
        "value": [
          2167,
          -1339
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "高科西路",
        "value": [
          2132,
          -1398
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东明路",
        "value": [
          2075,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "高青路",
        "value": [
          2033,
          -1525
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "华夏西路",
        "value": [
          2033,
          -1604
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上南路",
        "value": [
          1997,
          -1648
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "灵岩南路",
        "value": [
          1891,
          -1648
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东方体育中心",
        "value": [
          1764,
          -1648
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF3265",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁7号线",
    "tooltip": {
      "formatter": "{b}:地铁7号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1533,
      -489
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#FF7F00",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#FF7F00",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "美兰湖",
        "value": [
          1603,
          -224
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "罗南新村",
        "value": [
          1603,
          -259
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "潘广路",
        "value": [
          1603,
          -294
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "刘行",
        "value": [
          1603,
          -329
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "顾村公园",
        "value": [
          1603,
          -362
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "祁华路",
        "value": [
          1603,
          -397
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海大学",
        "value": [
          1603,
          -431
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南陈路",
        "value": [
          1603,
          -466
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上大路",
        "value": [
          1603,
          -500
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "场中路",
        "value": [
          1603,
          -535
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大场镇",
        "value": [
          1603,
          -569
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "行知路",
        "value": [
          1603,
          -604
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大华三路",
        "value": [
          1603,
          -638
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "新村路",
        "value": [
          1603,
          -673
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "岚皋路",
        "value": [
          1603,
          -707
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "镇坪路",
        "value": [
          1603,
          -764
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "长寿路",
        "value": [
          1651,
          -815
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "昌平路",
        "value": [
          1706,
          -870
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "静安寺",
        "value": [
          1760,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "常熟路",
        "value": [
          1763,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "肇嘉浜路",
        "value": [
          1762,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东安路",
        "value": [
          1763,
          -1240
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙华中路",
        "value": [
          1763,
          -1339
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "后滩",
        "value": [
          1833,
          -1397
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "长清路",
        "value": [
          1911,
          -1397
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "耀华路",
        "value": [
          1989,
          -1396
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "云台路",
        "value": [
          2056,
          -1398
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "高科西路",
        "value": [
          2132,
          -1398
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "杨高南路",
        "value": [
          2234,
          -1398
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "锦绣路",
        "value": [
          2280,
          -1398
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "芳华路",
        "value": [
          2370,
          -1309
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙阳路",
        "value": [
          2449,
          -1229
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "花木路",
        "value": [
          2506,
          -1172
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#FF7F00",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁8号线",
    "tooltip": {
      "formatter": "{b}:地铁8号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      2314,
      -347
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#4f90cb",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#4f90cb",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "市光路",
        "value": [
          2375,
          -188
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "嫩江路",
        "value": [
          2375,
          -246
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "翔殷路",
        "value": [
          2375,
          -301
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "黄兴公园",
        "value": [
          2375,
          -356
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "延吉中路",
        "value": [
          2375,
          -414
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "黄兴路",
        "value": [
          2341,
          -449
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江浦路",
        "value": [
          2308,
          -482
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "鞍山新村",
        "value": [
          2274,
          -516
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "四平路",
        "value": [
          2241,
          -549
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "曲阳路",
        "value": [
          2207,
          -583
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹口足球场",
        "value": [
          2086,
          -623
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "西藏北路",
        "value": [
          1999,
          -622
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中兴路",
        "value": [
          1943,
          -671
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "曲阜路",
        "value": [
          1943,
          -843
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "人民广场",
        "value": [
          1943,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大世界",
        "value": [
          1989,
          -994
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "老西门",
        "value": [
          1989,
          -1050
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陆家浜路",
        "value": [
          1989,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "西藏南路",
        "value": [
          1989,
          -1240
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中华艺术宫",
        "value": [
          1989,
          -1353
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "耀华路",
        "value": [
          1989,
          -1396
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "成山路",
        "value": [
          1989,
          -1440
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "杨思",
        "value": [
          1912,
          -1501
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东方体育中心",
        "value": [
          1764,
          -1648
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "凌兆新村",
        "value": [
          1764,
          -1689
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "芦恒路",
        "value": [
          1764,
          -1738
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "浦江镇",
        "value": [
          1764,
          -1787
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江月路",
        "value": [
          1764,
          -1837
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "联航路",
        "value": [
          1764,
          -1885
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "沈杜公路",
        "value": [
          1764,
          -1934
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#4f90cb",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "浦江线",
    "tooltip": {
      "formatter": "{b}:浦江线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1859,
      -2255
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#9ba0ad",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#9ba0ad",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "沈杜公路",
        "value": [
          1764,
          -1934
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#9ba0ad",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "三鲁公路",
        "value": [
          1842,
          -2020
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#9ba0ad",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "闵瑞路",
        "value": [
          1945,
          -2085
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#9ba0ad",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "浦航路",
        "value": [
          1945,
          -2163
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#9ba0ad",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东城一路",
        "value": [
          1945,
          -2241
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#9ba0ad",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "汇臻路",
        "value": [
          1945,
          -2319
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#9ba0ad",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁9号线",
    "tooltip": {
      "formatter": "{b}:地铁9号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1274,
      -1087
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#95d3db",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#95d3db",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "曹路",
        "value": [
          3261,
          -729
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "民雷路",
        "value": [
          3144,
          -729
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "顾唐路",
        "value": [
          3001,
          -729
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金海路",
        "value": [
          2884,
          -729
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金吉路",
        "value": [
          2756,
          -729
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金桥",
        "value": [
          2626,
          -729
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "台儿庄路",
        "value": [
          2535,
          -825
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "蓝天路",
        "value": [
          2483,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "芳甸路",
        "value": [
          2392,
          -968
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "杨高中路",
        "value": [
          2301,
          -1053
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "世纪大道",
        "value": [
          2222,
          -1022
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "商城路",
        "value": [
          2145,
          -1022
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "小南门",
        "value": [
          2054,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陆家浜路",
        "value": [
          1989,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "马当路",
        "value": [
          1956,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "打浦桥",
        "value": [
          1878,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "嘉善路",
        "value": [
          1820,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "肇嘉浜路",
        "value": [
          1762,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "徐家汇",
        "value": [
          1666,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宜山路",
        "value": [
          1550,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "桂林路",
        "value": [
          1472,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "漕河泾开发区",
        "value": [
          1389,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "合川路",
        "value": [
          1303,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "星中路",
        "value": [
          1236,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "七宝",
        "value": [
          1169,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中春路",
        "value": [
          1107,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "九亭",
        "value": [
          1024,
          -1145
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "泗泾",
        "value": [
          990,
          -1179
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "佘山",
        "value": [
          942,
          -1227
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "洞泾",
        "value": [
          892,
          -1277
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "松江大学城",
        "value": [
          843,
          -1327
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "松江新城",
        "value": [
          794,
          -1375
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "松江体育中心",
        "value": [
          794,
          -1427
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "醉白池",
        "value": [
          794,
          -1479
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "松江南站",
        "value": [
          794,
          -1531
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#95d3db",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "磁悬浮",
    "tooltip": {
      "formatter": "{b}:磁悬浮<br />"
    },
    "symbolSize": 0.1,
    "value": [
      2600,
      -1371
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#B3B3B3",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#B3B3B3",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "龙阳路",
        "value": [
          2449,
          -1229
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#B3B3B3",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "浦东国际机场",
        "value": [
          2743,
          -1534
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#B3B3B3",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁10号线(基隆路-航中路)",
    "tooltip": {
      "formatter": "{b}:地铁10号线(基隆路-航中路)<br />"
    },
    "symbolSize": 0.1,
    "value": [
      884,
      -1057
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#C9A7D5",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#C9A7D5",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "基隆路",
        "value": [
          2769,
          -219
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "港城路",
        "value": [
          2621,
          -219
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "高桥",
        "value": [
          2548,
          -117
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "高桥西",
        "value": [
          2470,
          -117
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "双江路",
        "value": [
          2392,
          -117
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "国帆路",
        "value": [
          2314,
          -117
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "新江湾城",
        "value": [
          2241,
          -293
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "殷高东路",
        "value": [
          2241,
          -329
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "三门路",
        "value": [
          2241,
          -364
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江湾体育场",
        "value": [
          2241,
          -399
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "五角场",
        "value": [
          2241,
          -434
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "国权路",
        "value": [
          2241,
          -469
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "同济大学",
        "value": [
          2241,
          -504
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "四平路",
        "value": [
          2241,
          -549
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "邮电新村",
        "value": [
          2199,
          -660
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "海伦路",
        "value": [
          2120,
          -738
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "四川北路",
        "value": [
          2072,
          -786
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "天潼路",
        "value": [
          2051,
          -843
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南京东路",
        "value": [
          2051,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "豫园",
        "value": [
          2051,
          -978
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "老西门",
        "value": [
          1989,
          -1050
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "一大会址·新天地",
        "value": [
          1893,
          -1050
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陕西南路",
        "value": [
          1810,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海图书馆",
        "value": [
          1666,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "交通大学",
        "value": [
          1596,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹桥路",
        "value": [
          1550,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宋园路",
        "value": [
          1442,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "伊犁路",
        "value": [
          1374,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "水城路",
        "value": [
          1307,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙溪路",
        "value": [
          1239,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙柏新村",
        "value": [
          1171,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "紫藤路",
        "value": [
          1103,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "航中路",
        "value": [
          1035,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁10号线(基隆路-虹桥火车站)",
    "tooltip": {
      "formatter": "{b}:地铁10号线(基隆路-虹桥火车站)<br />"
    },
    "symbolSize": 0.1,
    "value": [
      824,
      -959
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#C9A7D5",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#C9A7D5",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "基隆路",
        "value": [
          2769,
          -219
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "港城路",
        "value": [
          2621,
          -219
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "高桥",
        "value": [
          2548,
          -117
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "高桥西",
        "value": [
          2470,
          -117
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "双江路",
        "value": [
          2392,
          -117
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "国帆路",
        "value": [
          2314,
          -117
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "新江湾城",
        "value": [
          2241,
          -293
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "殷高东路",
        "value": [
          2241,
          -329
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "三门路",
        "value": [
          2241,
          -364
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江湾体育场",
        "value": [
          2241,
          -399
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "五角场",
        "value": [
          2241,
          -434
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "国权路",
        "value": [
          2241,
          -469
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "同济大学",
        "value": [
          2241,
          -504
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "四平路",
        "value": [
          2241,
          -549
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "邮电新村",
        "value": [
          2199,
          -660
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "海伦路",
        "value": [
          2120,
          -738
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "四川北路",
        "value": [
          2072,
          -786
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "天潼路",
        "value": [
          2051,
          -843
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南京东路",
        "value": [
          2051,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "豫园",
        "value": [
          2051,
          -978
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "老西门",
        "value": [
          1989,
          -1050
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "一大会址·新天地",
        "value": [
          1893,
          -1050
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陕西南路",
        "value": [
          1810,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海图书馆",
        "value": [
          1666,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "交通大学",
        "value": [
          1596,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹桥路",
        "value": [
          1550,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宋园路",
        "value": [
          1442,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "伊犁路",
        "value": [
          1374,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "水城路",
        "value": [
          1307,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙溪路",
        "value": [
          1239,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海动物园",
        "value": [
          1206,
          -992
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹桥1号航站楼",
        "value": [
          1190,
          -971
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹桥2号航站楼",
        "value": [
          1157,
          -934
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹桥火车站",
        "value": [
          1053,
          -934
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#C9A7D5",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁11号线(嘉定北-迪士尼)",
    "tooltip": {
      "formatter": "{b}:地铁11号线(嘉定北-迪士尼)<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1320,
      -234
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#800000",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#800000",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "迪士尼",
        "value": [
          2587,
          -1748
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "康新公路",
        "value": [
          2528,
          -1748
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "秀沿路",
        "value": [
          2476,
          -1722
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "罗山路",
        "value": [
          2437,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "御桥",
        "value": [
          2317,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "浦三路",
        "value": [
          2197,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "三林东",
        "value": [
          2077,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "三林",
        "value": [
          1951,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东方体育中心",
        "value": [
          1764,
          -1648
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙耀路",
        "value": [
          1666,
          -1472
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "云锦路",
        "value": [
          1666,
          -1399
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙华",
        "value": [
          1666,
          -1313
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海游泳馆",
        "value": [
          1666,
          -1193
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "徐家汇",
        "value": [
          1666,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "交通大学",
        "value": [
          1596,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江苏路",
        "value": [
          1668,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "隆德路",
        "value": [
          1606,
          -862
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "曹杨路",
        "value": [
          1546,
          -802
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "枫桥路",
        "value": [
          1496,
          -751
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "真如",
        "value": [
          1464,
          -719
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海西站",
        "value": [
          1429,
          -686
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "李子园",
        "value": [
          1399,
          -652
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "祁连山路",
        "value": [
          1399,
          -617
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "武威路",
        "value": [
          1399,
          -582
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "桃浦新村",
        "value": [
          1399,
          -548
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南翔",
        "value": [
          1399,
          -513
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陈翔公路",
        "value": [
          1399,
          -478
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "马陆",
        "value": [
          1399,
          -443
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "嘉定新城",
        "value": [
          1399,
          -408
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "白银路",
        "value": [
          1399,
          -343
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "嘉定西",
        "value": [
          1399,
          -303
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "嘉定北",
        "value": [
          1399,
          -259
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁11号线(花桥-迪士尼)",
    "tooltip": {
      "formatter": "{b}:地铁11号线(花桥-迪士尼)<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1114,
      -301
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#800000",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#800000",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "迪士尼",
        "value": [
          2587,
          -1748
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "康新公路",
        "value": [
          2528,
          -1748
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "秀沿路",
        "value": [
          2476,
          -1722
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "罗山路",
        "value": [
          2437,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "御桥",
        "value": [
          2317,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "浦三路",
        "value": [
          2197,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "三林东",
        "value": [
          2077,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "三林",
        "value": [
          1951,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东方体育中心",
        "value": [
          1764,
          -1648
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙耀路",
        "value": [
          1666,
          -1472
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "云锦路",
        "value": [
          1666,
          -1399
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙华",
        "value": [
          1666,
          -1313
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海游泳馆",
        "value": [
          1666,
          -1193
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "徐家汇",
        "value": [
          1666,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "交通大学",
        "value": [
          1596,
          -1030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江苏路",
        "value": [
          1668,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "隆德路",
        "value": [
          1606,
          -862
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "曹杨路",
        "value": [
          1546,
          -802
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "枫桥路",
        "value": [
          1496,
          -751
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "真如",
        "value": [
          1464,
          -719
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海西站",
        "value": [
          1429,
          -686
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "李子园",
        "value": [
          1399,
          -652
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "祁连山路",
        "value": [
          1399,
          -617
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "武威路",
        "value": [
          1399,
          -582
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "桃浦新村",
        "value": [
          1399,
          -548
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南翔",
        "value": [
          1399,
          -513
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陈翔公路",
        "value": [
          1399,
          -478
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "马陆",
        "value": [
          1399,
          -443
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "嘉定新城",
        "value": [
          1399,
          -408
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海赛车场",
        "value": [
          1376,
          -387
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "昌吉东路",
        "value": [
          1352,
          -366
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海汽车城",
        "value": [
          1328,
          -345
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "安亭",
        "value": [
          1305,
          -323
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "兆丰路",
        "value": [
          1198,
          -323
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "光明路",
        "value": [
          1094,
          -323
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "花桥",
        "value": [
          988,
          -323
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#800000",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁12号线",
    "tooltip": {
      "formatter": "{b}:地铁12号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      2801,
      -676
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#0C785E",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#0C785E",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "金海路",
        "value": [
          2884,
          -729
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "申江路",
        "value": [
          2883,
          -637
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金京路",
        "value": [
          2801,
          -637
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "杨高北路",
        "value": [
          2725,
          -637
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "巨峰路",
        "value": [
          2608,
          -637
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东陆路",
        "value": [
          2550,
          -637
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "复兴岛",
        "value": [
          2441,
          -637
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "爱国路",
        "value": [
          2366,
          -637
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "隆昌路",
        "value": [
          2324,
          -684
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "宁国路",
        "value": [
          2288,
          -723
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江浦公园",
        "value": [
          2255,
          -762
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大连路",
        "value": [
          2222,
          -808
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "提篮桥",
        "value": [
          2180,
          -838
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "国际客运中心",
        "value": [
          2125,
          -841
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "天潼路",
        "value": [
          2051,
          -843
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "曲阜路",
        "value": [
          1943,
          -843
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "汉中路",
        "value": [
          1837,
          -818
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南京西路",
        "value": [
          1810,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陕西南路",
        "value": [
          1810,
          -1021
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "嘉善路",
        "value": [
          1820,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大木桥路",
        "value": [
          1867,
          -1240
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙华中路",
        "value": [
          1763,
          -1339
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙华",
        "value": [
          1666,
          -1313
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "龙漕路",
        "value": [
          1615,
          -1295
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "漕宝路",
        "value": [
          1509,
          -1258
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "桂林公园",
        "value": [
          1417,
          -1256
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹漕路",
        "value": [
          1352,
          -1256
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹梅路",
        "value": [
          1292,
          -1256
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东兰路",
        "value": [
          1232,
          -1256
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "顾戴路",
        "value": [
          1175,
          -1256
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹莘路",
        "value": [
          1110,
          -1256
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "七莘路",
        "value": [
          1047,
          -1256
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#0C785E",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁13号线",
    "tooltip": {
      "formatter": "{b}:地铁13号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1153,
      -777
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#E796C1",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#E796C1",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "张江路",
        "value": [
          2641,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "学林路",
        "value": [
          2576,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "中科路",
        "value": [
          2511,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "华夏中路",
        "value": [
          2446,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "莲溪路",
        "value": [
          2400,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "陈春路",
        "value": [
          2335,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "北蔡",
        "value": [
          2270,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "下南路",
        "value": [
          2205,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "华鹏路",
        "value": [
          2140,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东明路",
        "value": [
          2075,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "成山路",
        "value": [
          1989,
          -1440
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "长清路",
        "value": [
          1911,
          -1397
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "世博大道",
        "value": [
          1956,
          -1365
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "世博会博物馆",
        "value": [
          1956,
          -1287
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "马当路",
        "value": [
          1956,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "一大会址·新天地",
        "value": [
          1893,
          -1050
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "淮海中路",
        "value": [
          1838,
          -997
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南京西路",
        "value": [
          1810,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "自然博物馆",
        "value": [
          1838,
          -892
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "汉中路",
        "value": [
          1837,
          -818
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "江宁路",
        "value": [
          1743,
          -818
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "长寿路",
        "value": [
          1651,
          -815
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "武宁路",
        "value": [
          1613,
          -837
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "隆德路",
        "value": [
          1606,
          -862
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金沙江路",
        "value": [
          1546,
          -850
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大渡河路",
        "value": [
          1481,
          -850
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "真北路",
        "value": [
          1429,
          -850
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "祁连山南路",
        "value": [
          1403,
          -824
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "丰庄",
        "value": [
          1364,
          -798
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金沙江西路",
        "value": [
          1299,
          -798
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "金运路",
        "value": [
          1221,
          -798
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#E796C1",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁15号线",
    "tooltip": {
      "formatter": "{b}:地铁15号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      1534,
      -2086
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#b9a583",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#b9a583",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "顾村公园",
        "value": [
          1603,
          -362
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "锦秋路",
        "value": [
          1495,
          -403
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "丰翔路",
        "value": [
          1495,
          -455
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "南大路",
        "value": [
          1495,
          -520
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "祁安路",
        "value": [
          1495,
          -572
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "古浪路",
        "value": [
          1495,
          -624
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "武威东路",
        "value": [
          1462,
          -663
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海西站",
        "value": [
          1429,
          -686
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "铜川路",
        "value": [
          1378,
          -728
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "梅岭北路",
        "value": [
          1417,
          -767
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "大渡河路",
        "value": [
          1481,
          -850
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "长风公园",
        "value": [
          1475,
          -890
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "娄山关路",
        "value": [
          1475,
          -926
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "红宝石路",
        "value": [
          1462,
          -955
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "姚虹路",
        "value": [
          1443,
          -981
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "吴中路",
        "value": [
          1423,
          -1007
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "桂林路",
        "value": [
          1472,
          -1105
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "桂林公园",
        "value": [
          1417,
          -1256
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "上海南站",
        "value": [
          1509,
          -1402
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "华东理工大学",
        "value": [
          1509,
          -1482
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "罗秀路",
        "value": [
          1509,
          -1560
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "朱梅路",
        "value": [
          1509,
          -1638
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "华泾西",
        "value": [
          1509,
          -1716
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "虹梅南路",
        "value": [
          1509,
          -1794
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "景西路",
        "value": [
          1469,
          -1820
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "曙建路",
        "value": [
          1430,
          -1872
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "双柏路",
        "value": [
          1430,
          -1937
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "元江路",
        "value": [
          1430,
          -2002
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "永德路",
        "value": [
          1430,
          -2067
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "紫竹高新区",
        "value": [
          1430,
          -2132
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#b9a583",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁16号线",
    "tooltip": {
      "formatter": "{b}:地铁16号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      2995,
      -2144
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#77C8C7",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#77C8C7",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "龙阳路",
        "value": [
          2449,
          -1229
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "华夏中路",
        "value": [
          2446,
          -1456
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "罗山路",
        "value": [
          2437,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "周浦东",
        "value": [
          2433,
          -1784
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "鹤沙航城",
        "value": [
          2433,
          -1877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "航头东",
        "value": [
          2433,
          -1970
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "新场",
        "value": [
          2473,
          -2030
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "野生动物园",
        "value": [
          2552,
          -2029
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "惠南",
        "value": [
          2639,
          -2028
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "惠南东",
        "value": [
          2639,
          -2113
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "书院",
        "value": [
          2751,
          -2139
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "临港大道",
        "value": [
          2855,
          -2139
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "滴水湖",
        "value": [
          2959,
          -2139
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#77C8C7",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁17号线",
    "tooltip": {
      "formatter": "{b}:地铁17号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      527,
      -819
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#c07774",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#c07774",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "虹桥火车站",
        "value": [
          1053,
          -933
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "诸光路",
        "value": [
          936,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "蟠龙路",
        "value": [
          858,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "徐盈路",
        "value": [
          780,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "徐泾北城",
        "value": [
          702,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "嘉松中路",
        "value": [
          624,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "赵巷",
        "value": [
          559,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "汇金路",
        "value": [
          481,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "青浦新城",
        "value": [
          403,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "漕盈路",
        "value": [
          325,
          -877
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "淀山湖大道",
        "value": [
          286,
          -951
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "朱家角",
        "value": [
          247,
          -1027
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "东方绿舟",
        "value": [
          169,
          -1027
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#c07774",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  },
  {
    "name": "地铁18号线",
    "tooltip": {
      "formatter": "{b}:地铁18号线<br />"
    },
    "symbolSize": 0.1,
    "value": [
      2301,
      -2281
    ],
    "fixed": true,
    "category": 1,
    "label": {
      "color": "#d9a972",
      "position": "bottom",
      "fontSize": 11,
      "fontWeight": 700
    },
    "itemStyle": {
      "normal": {
        "normal": {
          "color": "#fff",
          "borderColor": "#d9a972",
          "borderWidth": 3
        }
      }
    },
    "stations": [
      {
        "name": "御桥",
        "value": [
          2317,
          -1696
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#d9a972",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "康桥",
        "value": [
          2317,
          -1768
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#d9a972",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "周浦",
        "value": [
          2317,
          -1846
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#d9a972",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "繁荣路",
        "value": [
          2317,
          -1924
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#d9a972",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "沈梅路",
        "value": [
          2317,
          -2002
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#d9a972",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "鹤涛路",
        "value": [
          2317,
          -2080
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#d9a972",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "下沙",
        "value": [
          2317,
          -2158
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#d9a972",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      },
      {
        "name": "航头",
        "value": [
          2317,
          -2236
        ],
        "fixed": true,
        "symbol": "circle",
        "symbolSize": [
          10,
          10
        ],
        "label": {
          "color": "#000",
          "position": "top",
          "fontSize": 11
        },
        "category": 1,
        "itemStyle": {
          "normal": {
            "color": "#fff",
            "borderColor": "#d9a972",
            "borderWidth": 2
          }
        },
        "tooltip": {
          "formatter": "{b}"
        }
      }
    ]
  }
]

// 广州站点
export const guangzhouLine = [
  {
    source: '西塱',
    target: '坑口',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '坑口',
    target: '花地湾',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '花地湾',
    target: '芳村',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '芳村',
    target: '黄沙',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '黄沙',
    target: '长寿路',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '长寿路',
    target: '陈家祠',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '陈家祠',
    target: '西门口',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '西门口',
    target: '公园前',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '公园前',
    target: '农讲所',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '农讲所',
    target: '烈士陵园',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '烈士陵园',
    target: '东山口',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '东山口',
    target: '杨箕',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '杨箕',
    target: '体育西路',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '体育西路',
    target: '体育中心',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },
  {
    source: '体育中心',
    target: '广州东站',
    lineStyle: {
      normal: {
        color: '#F9E103',
      },
    },
  },

  {
    source: '嘉禾望岗',
    target: '黄边',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '黄边',
    target: '江夏',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '江夏',
    target: '萧岗',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '萧岗',
    target: '白云文化广场',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '白云文化广场',
    target: '白云公园',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '白云公园',
    target: '飞翔公园',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '飞翔公园',
    target: '三元里',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '三元里',
    target: '广州火车站',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '广州火车站',
    target: '越秀公园',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '越秀公园',
    target: '纪念堂',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '纪念堂',
    target: '公园前',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '公园前',
    target: '海珠广场',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '海珠广场',
    target: '市二宫',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '市二宫',
    target: '江南西',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '江南西',
    target: '昌岗',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '昌岗',
    target: '江泰路',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '江泰路',
    target: '东晓南',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '东晓南',
    target: '南洲',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '南洲',
    target: '洛溪',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '洛溪',
    target: '南浦',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '南浦',
    target: '会江',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '会江',
    target: '石壁',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },
  {
    source: '石壁',
    target: '广州南站',
    lineStyle: {
      normal: {
        color: '#4f90cb',
      },
    },
  },

  {
    source: '天河客运站',
    target: '五山',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '五山',
    target: '华师',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '华师',
    target: '岗顶',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '岗顶',
    target: '石牌桥',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '石牌桥',
    target: '体育西路',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '体育西路',
    target: '珠江新城',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '珠江新城',
    target: '广州塔',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '广州塔',
    target: '客村',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '客村',
    target: '大塘',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '大塘',
    target: '沥滘',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '沥滘',
    target: '厦滘',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '厦滘',
    target: '大石',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '大石',
    target: '汉溪长隆',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '汉溪长隆',
    target: '市桥',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '市桥',
    target: '番禺广场',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },

  {
    source: '体育西路',
    target: '林和西',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '林和西',
    target: '广州东站',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '广州东站',
    target: '燕塘',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '燕塘',
    target: '梅花园',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '梅花园',
    target: '京溪南方医院',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '京溪南方医院',
    target: '同和',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '同和',
    target: '永泰',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '永泰',
    target: '白云大道北',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '白云大道北',
    target: '嘉禾望岗',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '嘉禾望岗',
    target: '龙归',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '龙归',
    target: '人和',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '人和',
    target: '高增',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '高增',
    target: '机场南(1号航站楼)',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },
  {
    source: '机场南(1号航站楼)',
    target: '机场北(2号航站楼)',
    lineStyle: {
      normal: {
        color: '#EA6632',
      },
    },
  },

  {
    source: '黄村',
    target: '车陂',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '车陂',
    target: '车陂南',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '车陂南',
    target: '万胜围',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '万胜围',
    target: '官洲',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '官洲',
    target: '大学城北',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '大学城北',
    target: '大学城南',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '大学城南',
    target: '新造',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '新造',
    target: '石碁',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '石碁',
    target: '海傍',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '海傍',
    target: '低涌',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '低涌',
    target: '东涌',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '东涌',
    target: '庆盛',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '庆盛',
    target: '黄阁汽车城',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '黄阁汽车城',
    target: '黄阁',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '黄阁',
    target: '蕉门',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '蕉门',
    target: '金洲',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '金洲',
    target: '飞沙角',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '飞沙角',
    target: '广隆',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '广隆',
    target: '大涌',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '大涌',
    target: '塘坑',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '塘坑',
    target: '南横',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },
  {
    source: '南横',
    target: '南沙客运港',
    lineStyle: {
      normal: {
        color: '#009900',
      },
    },
  },

  {
    source: '滘口',
    target: '坦尾',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '坦尾',
    target: '中山八',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '中山八',
    target: '西场',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '西场',
    target: '西村',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '西村',
    target: '广州火车站',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '广州火车站',
    target: '小北',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '小北',
    target: '淘金',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '淘金',
    target: '区庄',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '区庄',
    target: '动物园',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '动物园',
    target: '杨箕',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '杨箕',
    target: '五羊邨',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '五羊邨',
    target: '珠江新城',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '珠江新城',
    target: '猎德',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '猎德',
    target: '潭村',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '潭村',
    target: '员村',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '员村',
    target: '科韵路',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '科韵路',
    target: '车陂南',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '车陂南',
    target: '东圃',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '东圃',
    target: '三溪',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '三溪',
    target: '鱼珠',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '鱼珠',
    target: '大沙地',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '大沙地',
    target: '大沙东',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },
  {
    source: '大沙东',
    target: '文冲',
    lineStyle: {
      normal: {
        color: '#FF0000',
      },
    },
  },

  {
    source: '香雪',
    target: '萝岗',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '萝岗',
    target: '苏元',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '苏元',
    target: '暹岗',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '暹岗',
    target: '金峰',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '金峰',
    target: '黄陂',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '黄陂',
    target: '高塘石',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '高塘石',
    target: '柯木塱',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '柯木塱',
    target: '龙洞',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '龙洞',
    target: '植物园',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '植物园',
    target: '长湴',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '长湴',
    target: '天河客运站',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '天河客运站',
    target: '燕塘',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '燕塘',
    target: '天平架',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '天平架',
    target: '沙河顶',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '沙河顶',
    target: '黄花岗',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '黄花岗',
    target: '区庄',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '区庄',
    target: '东山口',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '东山口',
    target: '东湖',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '东湖',
    target: '团一大广场',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '团一大广场',
    target: '北京路',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '北京路',
    target: '海珠广场',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '海珠广场',
    target: '一德路',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '一德路',
    target: '文化公园',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '文化公园',
    target: '黄沙',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '黄沙',
    target: '如意坊',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '如意坊',
    target: '坦尾',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '坦尾',
    target: '河沙',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '河沙',
    target: '沙贝',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '沙贝',
    target: '横沙',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },
  {
    source: '横沙',
    target: '浔峰岗',
    lineStyle: {
      normal: {
        color: '#8B1F5C',
      },
    },
  },

  {
    source: '广州南站',
    target: '石壁',
    lineStyle: {
      normal: {
        color: '#3FA37E',
      },
    },
  },
  {
    source: '石壁',
    target: '谢村',
    lineStyle: {
      normal: {
        color: '#3FA37E',
      },
    },
  },
  {
    source: '谢村',
    target: '钟村',
    lineStyle: {
      normal: {
        color: '#3FA37E',
      },
    },
  },
  {
    source: '钟村',
    target: '汉溪长隆',
    lineStyle: {
      normal: {
        color: '#3FA37E',
      },
    },
  },
  {
    source: '汉溪长隆',
    target: '南村万博',
    lineStyle: {
      normal: {
        color: '#3FA37E',
      },
    },
  },
  {
    source: '南村万博',
    target: '员岗',
    lineStyle: {
      normal: {
        color: '#3FA37E',
      },
    },
  },
  {
    source: '员岗',
    target: '板桥',
    lineStyle: {
      normal: {
        color: '#3FA37E',
      },
    },
  },
  {
    source: '板桥',
    target: '大学城南',
    lineStyle: {
      normal: {
        color: '#3FA37E',
      },
    },
  },

  {
    source: '滘心',
    target: '亭岗',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '亭岗',
    target: '石井',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '石井',
    target: '小坪',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '小坪',
    target: '石潭',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '石潭',
    target: '聚龙',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '聚龙',
    target: '上步',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '上步',
    target: '同德',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '同德',
    target: '鹅掌坦',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '鹅掌坦',
    target: '西村',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '西村',
    target: '陈家祠',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '陈家祠',
    target: '华林寺',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '华林寺',
    target: '文化公园',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '文化公园',
    target: '同福西',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '同福西',
    target: '凤凰新村',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '凤凰新村',
    target: '沙园',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '沙园',
    target: '宝岗大道',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '宝岗大道',
    target: '昌岗',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '昌岗',
    target: '晓港',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '晓港',
    target: '中大',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '中大',
    target: '鹭江',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '鹭江',
    target: '客村',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '客村',
    target: '赤岗',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '赤岗',
    target: '磨碟沙',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '磨碟沙',
    target: '新港东',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '新港东',
    target: '琶洲',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },
  {
    source: '琶洲',
    target: '万胜围',
    lineStyle: {
      normal: {
        color: '#00A1CC',
      },
    },
  },

  {
    source: '高增',
    target: '清塘',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },
  {
    source: '清塘',
    target: '清布',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },
  {
    source: '清布',
    target: '莲塘',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },
  {
    source: '莲塘',
    target: '马鞍山公园',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },
  {
    source: '马鞍山公园',
    target: '花都广场',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },
  {
    source: '花都广场',
    target: '花果山公园',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },
  {
    source: '花果山公园',
    target: '花城路',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },
  {
    source: '花城路',
    target: '广州北站',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },
  {
    source: '广州北站',
    target: '花都汽车城',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },
  {
    source: '花都汽车城',
    target: '飞鹅岭',
    lineStyle: {
      normal: {
        color: '#99c857',
      },
    },
  },

  {
    source: '鱼珠',
    target: '裕丰围',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },
  {
    source: '裕丰围',
    target: '双岗',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },
  {
    source: '双岗',
    target: '南海神庙',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },
  {
    source: '南海神庙',
    target: '夏园',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },
  {
    source: '夏园',
    target: '南岗',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },
  {
    source: '南岗',
    target: '沙村',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },
  {
    source: '沙村',
    target: '白江',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },
  {
    source: '白江',
    target: '新塘',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },
  {
    source: '新塘',
    target: '官湖',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },
  {
    source: '官湖',
    target: '新沙',
    lineStyle: {
      normal: {
        color: '#a7b942',
      },
    },
  },

  {
    source: '嘉禾望岗',
    target: '白云东平',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '白云东平',
    target: '夏良',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '夏良',
    target: '太和',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '太和',
    target: '竹料',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '竹料',
    target: '钟落潭',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '钟落潭',
    target: '马沥',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '马沥',
    target: '新和',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '新和',
    target: '太平',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '太平',
    target: '神岗',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '神岗',
    target: '赤草',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '赤草',
    target: '从化客运站',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '从化客运站',
    target: '东风',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },

  {
    source: '员村',
    target: '天河公园',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '天河公园',
    target: '棠东',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '棠东',
    target: '黄村',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '黄村',
    target: '大观南路',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '大观南路',
    target: '天河智慧城',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '天河智慧城',
    target: '神舟路',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '神舟路',
    target: '科学城',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '科学城',
    target: '苏元',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '苏元',
    target: '水西',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '水西',
    target: '长平',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '长平',
    target: '金坑',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '金坑',
    target: '镇龙西',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '镇龙西',
    target: '镇龙',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '镇龙',
    target: '中新',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '中新',
    target: '坑贝',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '坑贝',
    target: '凤岗',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '凤岗',
    target: '朱村',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '朱村',
    target: '山田',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '山田',
    target: '钟岗',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },
  {
    source: '钟岗',
    target: '增城广场',
    lineStyle: {
      normal: {
        color: '#203082',
      },
    },
  },

  {
    source: '沥滘',
    target: '南洲',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '南洲',
    target: '石溪',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '石溪',
    target: '燕岗',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '燕岗',
    target: '沙园',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '沙园',
    target: '沙涌',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '沙涌',
    target: '鹤洞',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '鹤洞',
    target: '西塱',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '西塱',
    target: '菊树',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '菊树',
    target: '龙溪',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '龙溪',
    target: '金融高新区',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '金融高新区',
    target: '千灯湖',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '千灯湖',
    target: '礌岗',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '礌岗',
    target: '南桂路',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '南桂路',
    target: '桂城',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '桂城',
    target: '朝安',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '朝安',
    target: '普君北路',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '普君北路',
    target: '祖庙',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '祖庙',
    target: '同济路',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '同济路',
    target: '季华园',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '季华园',
    target: '魁奇路',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '魁奇路',
    target: '澜石',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '澜石',
    target: '世纪莲',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '世纪莲',
    target: '东平',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },
  {
    source: '东平',
    target: '新城东',
    lineStyle: {
      normal: {
        color: '#c9e887',
      },
    },
  },

  {
    source: '林和西',
    target: '体育中心南',
    lineStyle: {
      normal: {
        color: '#00a1cb',
      },
    },
  },
  {
    source: '体育中心南',
    target: '天河南',
    lineStyle: {
      normal: {
        color: '#00a1cb',
      },
    },
  },
  {
    source: '天河南',
    target: '黄埔大道',
    lineStyle: {
      normal: {
        color: '#00a1cb',
      },
    },
  },
  {
    source: '黄埔大道',
    target: '妇儿中心',
    lineStyle: {
      normal: {
        color: '#00a1cb',
      },
    },
  },
  {
    source: '妇儿中心',
    target: '花城大道',
    lineStyle: {
      normal: {
        color: '#00a1cb',
      },
    },
  },
  {
    source: '花城大道',
    target: '大剧院',
    lineStyle: {
      normal: {
        color: '#00a1cb',
      },
    },
  },
  {
    source: '大剧院',
    target: '海心沙',
    lineStyle: {
      normal: {
        color: '#00a1cb',
      },
    },
  },
  {
    source: '海心沙',
    target: '广州塔',
    lineStyle: {
      normal: {
        color: '#00a1cb',
      },
    },
  },

  {
    source: '新和',
    target: '红卫',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '红卫',
    target: '新南',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '新南',
    target: '枫下',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '枫下',
    target: '知识城',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '知识城',
    target: '何棠下',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '何棠下',
    target: '旺村',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '旺村',
    target: '汤村',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '汤村',
    target: '镇龙北',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },
  {
    source: '镇龙北',
    target: '镇龙',
    lineStyle: {
      normal: {
        color: '#721920',
      },
    },
  },

  {
    source: '冼村',
    target: '磨碟沙',
    lineStyle: {
      normal: {
        color: '#0056B4',
      },
    },
  },
  {
    source: '磨碟沙',
    target: '龙潭',
    lineStyle: {
      normal: {
        color: '#0056B4',
      },
    },
  },
  {
    source: '龙潭',
    target: '沙溪',
    lineStyle: {
      normal: {
        color: '#0056B4',
      },
    },
  },
  {
    source: '沙溪',
    target: '南村万博',
    lineStyle: {
      normal: {
        color: '#0056B4',
      },
    },
  },
  {
    source: '南村万博',
    target: '番禺广场',
    lineStyle: {
      normal: {
        color: '#0056B4',
      },
    },
  },
  {
    source: '番禺广场',
    target: '横沥',
    lineStyle: {
      normal: {
        color: '#0056B4',
      },
    },
  },
  {
    source: '横沥',
    target: '万顷沙',
    lineStyle: {
      normal: {
        color: '#0056B4',
      },
    },
  },
];

// 上海站点
export const shanghaiLine = [
  {
    "source": "富锦路",
    "target": "友谊西路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "友谊西路",
    "target": "宝安公路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "宝安公路",
    "target": "共富新村",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "共富新村",
    "target": "呼兰路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "呼兰路",
    "target": "通河新村",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "通河新村",
    "target": "共康路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "共康路",
    "target": "彭浦新村",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "彭浦新村",
    "target": "汶水路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "汶水路",
    "target": "上海马戏城",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "上海马戏城",
    "target": "延长路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "延长路",
    "target": "中山北路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "中山北路",
    "target": "上海火车站",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "上海火车站",
    "target": "汉中路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "汉中路",
    "target": "新闸路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "新闸路",
    "target": "人民广场",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "人民广场",
    "target": "一大会址·黄陂南路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "一大会址·黄陂南路",
    "target": "陕西南路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "陕西南路",
    "target": "常熟路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "常熟路",
    "target": "衡山路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "衡山路",
    "target": "徐家汇",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "徐家汇",
    "target": "上海体育馆",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "上海体育馆",
    "target": "漕宝路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "漕宝路",
    "target": "上海南站",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "上海南站",
    "target": "锦江乐园",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "锦江乐园",
    "target": "莲花路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "莲花路",
    "target": "外环路",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "外环路",
    "target": "莘庄",
    "lineStyle": {
      "normal": {
        "color": "#e8787a"
      }
    }
  },
  {
    "source": "徐泾东",
    "target": "虹桥火车站",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "虹桥火车站",
    "target": "虹桥2号航站楼",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "虹桥2号航站楼",
    "target": "淞虹路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "淞虹路",
    "target": "北新泾",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "北新泾",
    "target": "威宁路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "威宁路",
    "target": "娄山关路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "娄山关路",
    "target": "中山公园",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "中山公园",
    "target": "江苏路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "江苏路",
    "target": "静安寺",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "静安寺",
    "target": "南京西路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "南京西路",
    "target": "人民广场",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "人民广场",
    "target": "南京东路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "南京东路",
    "target": "陆家嘴",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "陆家嘴",
    "target": "东昌路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "东昌路",
    "target": "世纪大道",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "世纪大道",
    "target": "上海科技馆",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "上海科技馆",
    "target": "世纪公园",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "世纪公园",
    "target": "龙阳路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "龙阳路",
    "target": "张江高科",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "张江高科",
    "target": "金科路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "金科路",
    "target": "广兰路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "广兰路",
    "target": "唐镇",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "唐镇",
    "target": "创新中路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "创新中路",
    "target": "华夏东路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "华夏东路",
    "target": "川沙",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "川沙",
    "target": "凌空路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "凌空路",
    "target": "远东大道",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "远东大道",
    "target": "海天三路",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "海天三路",
    "target": "浦东国际机场",
    "lineStyle": {
      "normal": {
        "color": "#009900"
      }
    }
  },
  {
    "source": "江杨北路",
    "target": "铁力路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "铁力路",
    "target": "友谊路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "友谊路",
    "target": "宝杨路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "宝杨路",
    "target": "水产路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "水产路",
    "target": "淞滨路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "淞滨路",
    "target": "张华浜",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "张华浜",
    "target": "淞发路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "淞发路",
    "target": "长江南路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "长江南路",
    "target": "殷高西路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "殷高西路",
    "target": "江湾镇",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "江湾镇",
    "target": "大柏树",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "大柏树",
    "target": "赤峰路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "赤峰路",
    "target": "虹口足球场",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "虹口足球场",
    "target": "东宝兴路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "东宝兴路",
    "target": "宝山路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "宝山路",
    "target": "上海火车站",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "上海火车站",
    "target": "中潭路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "中潭路",
    "target": "镇坪路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "镇坪路",
    "target": "曹杨路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "曹杨路",
    "target": "金沙江路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "金沙江路",
    "target": "中山公园",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "中山公园",
    "target": "延安西路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "延安西路",
    "target": "虹桥路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "虹桥路",
    "target": "宜山路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "宜山路",
    "target": "漕溪路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "漕溪路",
    "target": "龙漕路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "龙漕路",
    "target": "石龙路",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "石龙路",
    "target": "上海南站",
    "lineStyle": {
      "normal": {
        "color": "#F9E103"
      }
    }
  },
  {
    "source": "宝山路",
    "target": "上海火车站",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "上海火车站",
    "target": "中潭路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "中潭路",
    "target": "镇坪路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "镇坪路",
    "target": "曹杨路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "曹杨路",
    "target": "金沙江路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "金沙江路",
    "target": "中山公园",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "中山公园",
    "target": "延安西路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "延安西路",
    "target": "虹桥路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "虹桥路",
    "target": "宜山路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "宜山路",
    "target": "上海体育馆",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "上海体育馆",
    "target": "上海体育场",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "上海体育场",
    "target": "东安路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "东安路",
    "target": "大木桥路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "大木桥路",
    "target": "鲁班路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "鲁班路",
    "target": "西藏南路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "西藏南路",
    "target": "南浦大桥",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "南浦大桥",
    "target": "塘桥",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "塘桥",
    "target": "蓝村路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "蓝村路",
    "target": "浦电路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "浦电路",
    "target": "世纪大道",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "世纪大道",
    "target": "浦东大道",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "浦东大道",
    "target": "杨树浦路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "杨树浦路",
    "target": "大连路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "大连路",
    "target": "临平路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "临平路",
    "target": "海伦路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "海伦路",
    "target": "宝山路",
    "lineStyle": {
      "normal": {
        "color": "#660066"
      }
    }
  },
  {
    "source": "东川路",
    "target": "金平路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "金平路",
    "target": "华宁路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "华宁路",
    "target": "文井路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "文井路",
    "target": "闵行开发区",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "莘庄",
    "target": "春申路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "春申路",
    "target": "银都路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "银都路",
    "target": "颛桥",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "颛桥",
    "target": "北桥",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "北桥",
    "target": "剑川路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "剑川路",
    "target": "东川路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "东川路",
    "target": "江川路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "江川路",
    "target": "西渡",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "西渡",
    "target": "萧塘",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "萧塘",
    "target": "奉浦大道",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "奉浦大道",
    "target": "环城东路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "环城东路",
    "target": "望园路",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "望园路",
    "target": "金海湖",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "金海湖",
    "target": "奉贤新城",
    "lineStyle": {
      "normal": {
        "color": "#CC00CC"
      }
    }
  },
  {
    "source": "港城路",
    "target": "外高桥保税区北",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "外高桥保税区北",
    "target": "航津路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "航津路",
    "target": "外高桥保税区南",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "外高桥保税区南",
    "target": "洲海路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "洲海路",
    "target": "五洲大道",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "五洲大道",
    "target": "东靖路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "东靖路",
    "target": "巨峰路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "巨峰路",
    "target": "五莲路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "五莲路",
    "target": "博兴路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "博兴路",
    "target": "金桥路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "金桥路",
    "target": "云山路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "云山路",
    "target": "德平路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "德平路",
    "target": "北洋泾路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "北洋泾路",
    "target": "民生路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "民生路",
    "target": "源深体育中心",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "源深体育中心",
    "target": "世纪大道",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "世纪大道",
    "target": "浦电路⑥",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "浦电路⑥",
    "target": "蓝村路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "蓝村路",
    "target": "上海儿童医学中心",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "上海儿童医学中心",
    "target": "临沂新村",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "临沂新村",
    "target": "高科西路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "高科西路",
    "target": "东明路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "东明路",
    "target": "高青路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "高青路",
    "target": "华夏西路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "华夏西路",
    "target": "上南路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "上南路",
    "target": "灵岩南路",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "灵岩南路",
    "target": "东方体育中心",
    "lineStyle": {
      "normal": {
        "color": "#FF3265"
      }
    }
  },
  {
    "source": "美兰湖",
    "target": "罗南新村",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "罗南新村",
    "target": "潘广路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "潘广路",
    "target": "刘行",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "刘行",
    "target": "顾村公园",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "顾村公园",
    "target": "祁华路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "祁华路",
    "target": "上海大学",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "上海大学",
    "target": "南陈路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "南陈路",
    "target": "上大路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "上大路",
    "target": "场中路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "场中路",
    "target": "大场镇",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "大场镇",
    "target": "行知路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "行知路",
    "target": "大华三路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "大华三路",
    "target": "新村路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "新村路",
    "target": "岚皋路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "岚皋路",
    "target": "镇坪路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "镇坪路",
    "target": "长寿路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "长寿路",
    "target": "昌平路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "昌平路",
    "target": "静安寺",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "静安寺",
    "target": "常熟路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "常熟路",
    "target": "肇嘉浜路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "肇嘉浜路",
    "target": "东安路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "东安路",
    "target": "龙华中路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "龙华中路",
    "target": "后滩",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "后滩",
    "target": "长清路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "长清路",
    "target": "耀华路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "耀华路",
    "target": "云台路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "云台路",
    "target": "高科西路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "高科西路",
    "target": "杨高南路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "杨高南路",
    "target": "锦绣路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "锦绣路",
    "target": "芳华路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "芳华路",
    "target": "龙阳路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "龙阳路",
    "target": "花木路",
    "lineStyle": {
      "normal": {
        "color": "#FF7F00"
      }
    }
  },
  {
    "source": "市光路",
    "target": "嫩江路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "嫩江路",
    "target": "翔殷路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "翔殷路",
    "target": "黄兴公园",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "黄兴公园",
    "target": "延吉中路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "延吉中路",
    "target": "黄兴路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "黄兴路",
    "target": "江浦路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "江浦路",
    "target": "鞍山新村",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "鞍山新村",
    "target": "四平路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "四平路",
    "target": "曲阳路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "曲阳路",
    "target": "虹口足球场",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "虹口足球场",
    "target": "西藏北路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "西藏北路",
    "target": "中兴路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "中兴路",
    "target": "曲阜路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "曲阜路",
    "target": "人民广场",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "人民广场",
    "target": "大世界",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "大世界",
    "target": "老西门",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "老西门",
    "target": "陆家浜路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "陆家浜路",
    "target": "西藏南路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "西藏南路",
    "target": "中华艺术宫",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "中华艺术宫",
    "target": "耀华路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "耀华路",
    "target": "成山路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "成山路",
    "target": "杨思",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "杨思",
    "target": "东方体育中心",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "东方体育中心",
    "target": "凌兆新村",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "凌兆新村",
    "target": "芦恒路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "芦恒路",
    "target": "浦江镇",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "浦江镇",
    "target": "江月路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "江月路",
    "target": "联航路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "联航路",
    "target": "沈杜公路",
    "lineStyle": {
      "normal": {
        "color": "#4f90cb"
      }
    }
  },
  {
    "source": "沈杜公路",
    "target": "三鲁公路",
    "lineStyle": {
      "normal": {
        "color": "#9ba0ad"
      }
    }
  },
  {
    "source": "三鲁公路",
    "target": "闵瑞路",
    "lineStyle": {
      "normal": {
        "color": "#9ba0ad"
      }
    }
  },
  {
    "source": "闵瑞路",
    "target": "浦航路",
    "lineStyle": {
      "normal": {
        "color": "#9ba0ad"
      }
    }
  },
  {
    "source": "浦航路",
    "target": "东城一路",
    "lineStyle": {
      "normal": {
        "color": "#9ba0ad"
      }
    }
  },
  {
    "source": "东城一路",
    "target": "汇臻路",
    "lineStyle": {
      "normal": {
        "color": "#9ba0ad"
      }
    }
  },
  {
    "source": "曹路",
    "target": "民雷路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "民雷路",
    "target": "顾唐路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "顾唐路",
    "target": "金海路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "金海路",
    "target": "金吉路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "金吉路",
    "target": "金桥",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "金桥",
    "target": "台儿庄路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "台儿庄路",
    "target": "蓝天路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "蓝天路",
    "target": "芳甸路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "芳甸路",
    "target": "杨高中路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "杨高中路",
    "target": "世纪大道",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "世纪大道",
    "target": "商城路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "商城路",
    "target": "小南门",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "小南门",
    "target": "陆家浜路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "陆家浜路",
    "target": "马当路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "马当路",
    "target": "打浦桥",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "打浦桥",
    "target": "嘉善路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "嘉善路",
    "target": "肇嘉浜路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "肇嘉浜路",
    "target": "徐家汇",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "徐家汇",
    "target": "宜山路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "宜山路",
    "target": "桂林路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "桂林路",
    "target": "漕河泾开发区",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "漕河泾开发区",
    "target": "合川路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "合川路",
    "target": "星中路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "星中路",
    "target": "七宝",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "七宝",
    "target": "中春路",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "中春路",
    "target": "九亭",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "九亭",
    "target": "泗泾",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "泗泾",
    "target": "佘山",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "佘山",
    "target": "洞泾",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "洞泾",
    "target": "松江大学城",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "松江大学城",
    "target": "松江新城",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "松江新城",
    "target": "松江体育中心",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "松江体育中心",
    "target": "醉白池",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "醉白池",
    "target": "松江南站",
    "lineStyle": {
      "normal": {
        "color": "#95d3db"
      }
    }
  },
  {
    "source": "龙阳路",
    "target": "浦东国际机场",
    "lineStyle": {
      "normal": {
        "color": "#B3B3B3"
      }
    }
  },
  {
    "source": "基隆路",
    "target": "港城路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "港城路",
    "target": "高桥",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "高桥",
    "target": "高桥西",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "高桥西",
    "target": "双江路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "双江路",
    "target": "国帆路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "国帆路",
    "target": "新江湾城",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "新江湾城",
    "target": "殷高东路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "殷高东路",
    "target": "三门路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "三门路",
    "target": "江湾体育场",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "江湾体育场",
    "target": "五角场",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "五角场",
    "target": "国权路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "国权路",
    "target": "同济大学",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "同济大学",
    "target": "四平路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "四平路",
    "target": "邮电新村",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "邮电新村",
    "target": "海伦路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "海伦路",
    "target": "四川北路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "四川北路",
    "target": "天潼路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "天潼路",
    "target": "南京东路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "南京东路",
    "target": "豫园",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "豫园",
    "target": "老西门",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "老西门",
    "target": "一大会址·新天地",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "一大会址·新天地",
    "target": "陕西南路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "陕西南路",
    "target": "上海图书馆",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "上海图书馆",
    "target": "交通大学",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "交通大学",
    "target": "虹桥路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "虹桥路",
    "target": "宋园路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "宋园路",
    "target": "伊犁路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "伊犁路",
    "target": "水城路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "水城路",
    "target": "龙溪路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "龙溪路",
    "target": "龙柏新村",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "龙柏新村",
    "target": "紫藤路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "紫藤路",
    "target": "航中路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "基隆路",
    "target": "港城路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "港城路",
    "target": "高桥",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "高桥",
    "target": "高桥西",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "高桥西",
    "target": "双江路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "双江路",
    "target": "国帆路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "国帆路",
    "target": "新江湾城",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "新江湾城",
    "target": "殷高东路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "殷高东路",
    "target": "三门路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "三门路",
    "target": "江湾体育场",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "江湾体育场",
    "target": "五角场",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "五角场",
    "target": "国权路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "国权路",
    "target": "同济大学",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "同济大学",
    "target": "四平路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "四平路",
    "target": "邮电新村",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "邮电新村",
    "target": "海伦路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "海伦路",
    "target": "四川北路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "四川北路",
    "target": "天潼路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "天潼路",
    "target": "南京东路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "南京东路",
    "target": "豫园",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "豫园",
    "target": "老西门",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "老西门",
    "target": "一大会址·新天地",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "一大会址·新天地",
    "target": "陕西南路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "陕西南路",
    "target": "上海图书馆",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "上海图书馆",
    "target": "交通大学",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "交通大学",
    "target": "虹桥路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "虹桥路",
    "target": "宋园路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "宋园路",
    "target": "伊犁路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "伊犁路",
    "target": "水城路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "水城路",
    "target": "龙溪路",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "龙溪路",
    "target": "上海动物园",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "上海动物园",
    "target": "虹桥1号航站楼",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "虹桥1号航站楼",
    "target": "虹桥2号航站楼",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "虹桥2号航站楼",
    "target": "虹桥火车站",
    "lineStyle": {
      "normal": {
        "color": "#C9A7D5"
      }
    }
  },
  {
    "source": "迪士尼",
    "target": "康新公路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "康新公路",
    "target": "秀沿路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "秀沿路",
    "target": "罗山路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "罗山路",
    "target": "御桥",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "御桥",
    "target": "浦三路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "浦三路",
    "target": "三林东",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "三林东",
    "target": "三林",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "三林",
    "target": "东方体育中心",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "东方体育中心",
    "target": "龙耀路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "龙耀路",
    "target": "云锦路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "云锦路",
    "target": "龙华",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "龙华",
    "target": "上海游泳馆",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "上海游泳馆",
    "target": "徐家汇",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "徐家汇",
    "target": "交通大学",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "交通大学",
    "target": "江苏路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "江苏路",
    "target": "隆德路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "隆德路",
    "target": "曹杨路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "曹杨路",
    "target": "枫桥路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "枫桥路",
    "target": "真如",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "真如",
    "target": "上海西站",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "上海西站",
    "target": "李子园",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "李子园",
    "target": "祁连山路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "祁连山路",
    "target": "武威路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "武威路",
    "target": "桃浦新村",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "桃浦新村",
    "target": "南翔",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "南翔",
    "target": "陈翔公路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "陈翔公路",
    "target": "马陆",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "马陆",
    "target": "嘉定新城",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "嘉定新城",
    "target": "白银路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "白银路",
    "target": "嘉定西",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "嘉定西",
    "target": "嘉定北",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "迪士尼",
    "target": "康新公路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "康新公路",
    "target": "秀沿路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "秀沿路",
    "target": "罗山路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "罗山路",
    "target": "御桥",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "御桥",
    "target": "浦三路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "浦三路",
    "target": "三林东",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "三林东",
    "target": "三林",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "三林",
    "target": "东方体育中心",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "东方体育中心",
    "target": "龙耀路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "龙耀路",
    "target": "云锦路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "云锦路",
    "target": "龙华",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "龙华",
    "target": "上海游泳馆",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "上海游泳馆",
    "target": "徐家汇",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "徐家汇",
    "target": "交通大学",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "交通大学",
    "target": "江苏路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "江苏路",
    "target": "隆德路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "隆德路",
    "target": "曹杨路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "曹杨路",
    "target": "枫桥路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "枫桥路",
    "target": "真如",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "真如",
    "target": "上海西站",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "上海西站",
    "target": "李子园",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "李子园",
    "target": "祁连山路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "祁连山路",
    "target": "武威路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "武威路",
    "target": "桃浦新村",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "桃浦新村",
    "target": "南翔",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "南翔",
    "target": "陈翔公路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "陈翔公路",
    "target": "马陆",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "马陆",
    "target": "嘉定新城",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "嘉定新城",
    "target": "上海赛车场",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "上海赛车场",
    "target": "昌吉东路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "昌吉东路",
    "target": "上海汽车城",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "上海汽车城",
    "target": "安亭",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "安亭",
    "target": "兆丰路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "兆丰路",
    "target": "光明路",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "光明路",
    "target": "花桥",
    "lineStyle": {
      "normal": {
        "color": "#800000"
      }
    }
  },
  {
    "source": "金海路",
    "target": "申江路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "申江路",
    "target": "金京路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "金京路",
    "target": "杨高北路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "杨高北路",
    "target": "巨峰路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "巨峰路",
    "target": "东陆路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "东陆路",
    "target": "复兴岛",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "复兴岛",
    "target": "爱国路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "爱国路",
    "target": "隆昌路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "隆昌路",
    "target": "宁国路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "宁国路",
    "target": "江浦公园",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "江浦公园",
    "target": "大连路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "大连路",
    "target": "提篮桥",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "提篮桥",
    "target": "国际客运中心",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "国际客运中心",
    "target": "天潼路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "天潼路",
    "target": "曲阜路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "曲阜路",
    "target": "汉中路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "汉中路",
    "target": "南京西路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "南京西路",
    "target": "陕西南路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "陕西南路",
    "target": "嘉善路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "嘉善路",
    "target": "大木桥路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "大木桥路",
    "target": "龙华中路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "龙华中路",
    "target": "龙华",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "龙华",
    "target": "龙漕路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "龙漕路",
    "target": "漕宝路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "漕宝路",
    "target": "桂林公园",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "桂林公园",
    "target": "虹漕路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "虹漕路",
    "target": "虹梅路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "虹梅路",
    "target": "东兰路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "东兰路",
    "target": "顾戴路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "顾戴路",
    "target": "虹莘路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "虹莘路",
    "target": "七莘路",
    "lineStyle": {
      "normal": {
        "color": "#0C785E"
      }
    }
  },
  {
    "source": "张江路",
    "target": "学林路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "学林路",
    "target": "中科路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "中科路",
    "target": "华夏中路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "华夏中路",
    "target": "莲溪路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "莲溪路",
    "target": "陈春路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "陈春路",
    "target": "北蔡",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "北蔡",
    "target": "下南路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "下南路",
    "target": "华鹏路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "华鹏路",
    "target": "东明路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "东明路",
    "target": "成山路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "成山路",
    "target": "长清路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "长清路",
    "target": "世博大道",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "世博大道",
    "target": "世博会博物馆",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "世博会博物馆",
    "target": "马当路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "马当路",
    "target": "一大会址·新天地",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "一大会址·新天地",
    "target": "淮海中路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "淮海中路",
    "target": "南京西路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "南京西路",
    "target": "自然博物馆",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "自然博物馆",
    "target": "汉中路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "汉中路",
    "target": "江宁路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "江宁路",
    "target": "长寿路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "长寿路",
    "target": "武宁路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "武宁路",
    "target": "隆德路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "隆德路",
    "target": "金沙江路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "金沙江路",
    "target": "大渡河路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "大渡河路",
    "target": "真北路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "真北路",
    "target": "祁连山南路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "祁连山南路",
    "target": "丰庄",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "丰庄",
    "target": "金沙江西路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "金沙江西路",
    "target": "金运路",
    "lineStyle": {
      "normal": {
        "color": "#E796C1"
      }
    }
  },
  {
    "source": "顾村公园",
    "target": "锦秋路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "锦秋路",
    "target": "丰翔路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "丰翔路",
    "target": "南大路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "南大路",
    "target": "祁安路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "祁安路",
    "target": "古浪路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "古浪路",
    "target": "武威东路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "武威东路",
    "target": "上海西站",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "上海西站",
    "target": "铜川路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "铜川路",
    "target": "梅岭北路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "梅岭北路",
    "target": "大渡河路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "大渡河路",
    "target": "长风公园",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "长风公园",
    "target": "娄山关路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "娄山关路",
    "target": "红宝石路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "红宝石路",
    "target": "姚虹路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "姚虹路",
    "target": "吴中路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "吴中路",
    "target": "桂林路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "桂林路",
    "target": "桂林公园",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "桂林公园",
    "target": "上海南站",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "上海南站",
    "target": "华东理工大学",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "华东理工大学",
    "target": "罗秀路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "罗秀路",
    "target": "朱梅路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "朱梅路",
    "target": "华泾西",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "华泾西",
    "target": "虹梅南路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "虹梅南路",
    "target": "景西路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "景西路",
    "target": "曙建路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "曙建路",
    "target": "双柏路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "双柏路",
    "target": "元江路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "元江路",
    "target": "永德路",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "永德路",
    "target": "紫竹高新区",
    "lineStyle": {
      "normal": {
        "color": "#b9a583"
      }
    }
  },
  {
    "source": "龙阳路",
    "target": "华夏中路",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "华夏中路",
    "target": "罗山路",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "罗山路",
    "target": "周浦东",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "周浦东",
    "target": "鹤沙航城",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "鹤沙航城",
    "target": "航头东",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "航头东",
    "target": "新场",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "新场",
    "target": "野生动物园",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "野生动物园",
    "target": "惠南",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "惠南",
    "target": "惠南东",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "惠南东",
    "target": "书院",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "书院",
    "target": "临港大道",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "临港大道",
    "target": "滴水湖",
    "lineStyle": {
      "normal": {
        "color": "#77C8C7"
      }
    }
  },
  {
    "source": "虹桥火车站",
    "target": "诸光路",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "诸光路",
    "target": "蟠龙路",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "蟠龙路",
    "target": "徐盈路",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "徐盈路",
    "target": "徐泾北城",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "徐泾北城",
    "target": "嘉松中路",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "嘉松中路",
    "target": "赵巷",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "赵巷",
    "target": "汇金路",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "汇金路",
    "target": "青浦新城",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "青浦新城",
    "target": "漕盈路",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "漕盈路",
    "target": "淀山湖大道",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "淀山湖大道",
    "target": "朱家角",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "朱家角",
    "target": "东方绿舟",
    "lineStyle": {
      "normal": {
        "color": "#c07774"
      }
    }
  },
  {
    "source": "御桥",
    "target": "康桥",
    "lineStyle": {
      "normal": {
        "color": "#d9a972"
      }
    }
  },
  {
    "source": "康桥",
    "target": "周浦",
    "lineStyle": {
      "normal": {
        "color": "#d9a972"
      }
    }
  },
  {
    "source": "周浦",
    "target": "繁荣路",
    "lineStyle": {
      "normal": {
        "color": "#d9a972"
      }
    }
  },
  {
    "source": "繁荣路",
    "target": "沈梅路",
    "lineStyle": {
      "normal": {
        "color": "#d9a972"
      }
    }
  },
  {
    "source": "沈梅路",
    "target": "鹤涛路",
    "lineStyle": {
      "normal": {
        "color": "#d9a972"
      }
    }
  },
  {
    "source": "鹤涛路",
    "target": "下沙",
    "lineStyle": {
      "normal": {
        "color": "#d9a972"
      }
    }
  },
  {
    "source": "下沙",
    "target": "航头",
    "lineStyle": {
      "normal": {
        "color": "#d9a972"
      }
    }
  }
]

// export const guangzhouLine = 
// [
//   {
//     source: "西塱",
//     target: "坑口",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "坑口",
//     target: "花地湾",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "花地湾",
//     target: "芳村",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "芳村",
//     target: "黄沙",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "黄沙",
//     target: "长寿路",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "长寿路",
//     target: "陈家祠",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "陈家祠",
//     target: "西门口",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "西门口",
//     target: "公园前",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "公园前",
//     target: "农讲所",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "农讲所",
//     target: "烈士陵园",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "烈士陵园",
//     target: "东山口",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "东山口",
//     target: "杨箕",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "杨箕",
//     target: "体育西路",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "体育西路",
//     target: "体育中心",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "体育中心",
//     target: "广州东站",
//     lineStyle: {
//       normal: {
//         color: "#F9E103",
//       },
//     },
//   },
//   {
//     source: "嘉禾望岗",
//     target: "黄边",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "黄边",
//     target: "江夏",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "江夏",
//     target: "萧岗",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "萧岗",
//     target: "白云文化广场",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "白云文化广场",
//     target: "白云公园",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "白云公园",
//     target: "飞翔公园",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "飞翔公园",
//     target: "三元里",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "三元里",
//     target: "广州火车站",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "广州火车站",
//     target: "越秀公园",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "越秀公园",
//     target: "纪念堂",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "纪念堂",
//     target: "公园前",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "公园前",
//     target: "海珠广场",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "海珠广场",
//     target: "市二宫",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "市二宫",
//     target: "江南西",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "江南西",
//     target: "昌岗",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "昌岗",
//     target: "江泰路",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "江泰路",
//     target: "东晓南",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "东晓南",
//     target: "南洲",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "南洲",
//     target: "洛溪",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "洛溪",
//     target: "南浦",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "南浦",
//     target: "会江",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "会江",
//     target: "石壁",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "石壁",
//     target: "广州南站",
//     lineStyle: {
//       normal: {
//         color: "#4f90cb",
//       },
//     },
//   },
//   {
//     source: "天河客运站",
//     target: "五山",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "五山",
//     target: "华师",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "华师",
//     target: "岗顶",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "岗顶",
//     target: "石牌桥",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "石牌桥",
//     target: "体育西路",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "体育西路",
//     target: "珠江新城",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "珠江新城",
//     target: "广州塔",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "广州塔",
//     target: "客村",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "客村",
//     target: "大塘",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "大塘",
//     target: "沥滘",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "沥滘",
//     target: "厦滘",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "厦滘",
//     target: "大石",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "大石",
//     target: "汉溪长隆",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "汉溪长隆",
//     target: "市桥",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "市桥",
//     target: "番禺广场",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "体育西路",
//     target: "林和西",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "林和西",
//     target: "广州东站",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "广州东站",
//     target: "燕塘",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "燕塘",
//     target: "梅花园",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "梅花园",
//     target: "京溪南方医院",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "京溪南方医院",
//     target: "同和",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "同和",
//     target: "永泰",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "永泰",
//     target: "白云大道北",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "白云大道北",
//     target: "嘉禾望岗",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "嘉禾望岗",
//     target: "龙归",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "龙归",
//     target: "人和",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "人和",
//     target: "高增",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "高增",
//     target: "机场南(1号航站楼)",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "机场南(1号航站楼)",
//     target: "机场北(2号航站楼)",
//     lineStyle: {
//       normal: {
//         color: "#EA6632",
//       },
//     },
//   },
//   {
//     source: "黄村",
//     target: "车陂",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "车陂",
//     target: "车陂南",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "车陂南",
//     target: "万胜围",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "万胜围",
//     target: "官洲",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "官洲",
//     target: "大学城北",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "大学城北",
//     target: "大学城南",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "大学城南",
//     target: "新造",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "新造",
//     target: "石碁",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "石碁",
//     target: "海傍",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "海傍",
//     target: "低涌",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "低涌",
//     target: "东涌",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "东涌",
//     target: "庆盛",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "庆盛",
//     target: "黄阁汽车城",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "黄阁汽车城",
//     target: "黄阁",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "黄阁",
//     target: "蕉门",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "蕉门",
//     target: "金洲",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "金洲",
//     target: "飞沙角",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "飞沙角",
//     target: "广隆",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "广隆",
//     target: "大涌",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "大涌",
//     target: "塘坑",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "塘坑",
//     target: "南横",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "南横",
//     target: "南沙客运港",
//     lineStyle: {
//       normal: {
//         color: "#009900",
//       },
//     },
//   },
//   {
//     source: "滘口",
//     target: "坦尾",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "坦尾",
//     target: "中山八",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "中山八",
//     target: "西场",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "西场",
//     target: "西村",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "西村",
//     target: "广州火车站",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "广州火车站",
//     target: "小北",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "小北",
//     target: "淘金",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "淘金",
//     target: "区庄",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "区庄",
//     target: "动物园",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "动物园",
//     target: "杨箕",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "杨箕",
//     target: "五羊邨",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "五羊邨",
//     target: "珠江新城",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "珠江新城",
//     target: "猎德",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "猎德",
//     target: "潭村",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "潭村",
//     target: "员村",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "员村",
//     target: "科韵路",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "科韵路",
//     target: "车陂南",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "车陂南",
//     target: "东圃",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "东圃",
//     target: "三溪",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "三溪",
//     target: "鱼珠",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "鱼珠",
//     target: "大沙地",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "大沙地",
//     target: "大沙东",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "大沙东",
//     target: "文冲",
//     lineStyle: {
//       normal: {
//         color: "#FF0000",
//       },
//     },
//   },
//   {
//     source: "香雪",
//     target: "萝岗",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "萝岗",
//     target: "苏元",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "苏元",
//     target: "暹岗",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "暹岗",
//     target: "金峰",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "金峰",
//     target: "黄陂",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "黄陂",
//     target: "高塘石",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "高塘石",
//     target: "柯木塱",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "柯木塱",
//     target: "龙洞",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "龙洞",
//     target: "植物园",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "植物园",
//     target: "长湴",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "长湴",
//     target: "天河客运站",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "天河客运站",
//     target: "燕塘",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "燕塘",
//     target: "天平架",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "天平架",
//     target: "沙河顶",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "沙河顶",
//     target: "黄花岗",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "黄花岗",
//     target: "区庄",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "区庄",
//     target: "东山口",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "东山口",
//     target: "东湖",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "东湖",
//     target: "团一大广场",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "团一大广场",
//     target: "北京路",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "北京路",
//     target: "海珠广场",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "海珠广场",
//     target: "一德路",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "一德路",
//     target: "文化公园",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "文化公园",
//     target: "黄沙",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "黄沙",
//     target: "如意坊",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "如意坊",
//     target: "坦尾",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "坦尾",
//     target: "河沙",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "河沙",
//     target: "沙贝",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "沙贝",
//     target: "横沙",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "横沙",
//     target: "浔峰岗",
//     lineStyle: {
//       normal: {
//         color: "#8B1F5C",
//       },
//     },
//   },
//   {
//     source: "广州南站",
//     target: "石壁",
//     lineStyle: {
//       normal: {
//         color: "#3FA37E",
//       },
//     },
//   },
//   {
//     source: "石壁",
//     target: "谢村",
//     lineStyle: {
//       normal: {
//         color: "#3FA37E",
//       },
//     },
//   },
//   {
//     source: "谢村",
//     target: "钟村",
//     lineStyle: {
//       normal: {
//         color: "#3FA37E",
//       },
//     },
//   },
//   {
//     source: "钟村",
//     target: "汉溪长隆",
//     lineStyle: {
//       normal: {
//         color: "#3FA37E",
//       },
//     },
//   },
//   {
//     source: "汉溪长隆",
//     target: "南村万博",
//     lineStyle: {
//       normal: {
//         color: "#3FA37E",
//       },
//     },
//   },
//   {
//     source: "南村万博",
//     target: "员岗",
//     lineStyle: {
//       normal: {
//         color: "#3FA37E",
//       },
//     },
//   },
//   {
//     source: "员岗",
//     target: "板桥",
//     lineStyle: {
//       normal: {
//         color: "#3FA37E",
//       },
//     },
//   },
//   {
//     source: "板桥",
//     target: "大学城南",
//     lineStyle: {
//       normal: {
//         color: "#3FA37E",
//       },
//     },
//   },
//   {
//     source: "滘心",
//     target: "亭岗",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "亭岗",
//     target: "石井",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "石井",
//     target: "小坪",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "小坪",
//     target: "石潭",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "石潭",
//     target: "聚龙",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "聚龙",
//     target: "上步",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "上步",
//     target: "同德",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "同德",
//     target: "鹅掌坦",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "鹅掌坦",
//     target: "西村",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "西村",
//     target: "陈家祠",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "陈家祠",
//     target: "华林寺",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "华林寺",
//     target: "文化公园",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "文化公园",
//     target: "同福西",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "同福西",
//     target: "凤凰新村",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "凤凰新村",
//     target: "沙园",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "沙园",
//     target: "宝岗大道",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "宝岗大道",
//     target: "昌岗",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "昌岗",
//     target: "晓港",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "晓港",
//     target: "中大",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "中大",
//     target: "鹭江",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "鹭江",
//     target: "客村",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "客村",
//     target: "赤岗",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "赤岗",
//     target: "磨碟沙",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "磨碟沙",
//     target: "新港东",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "新港东",
//     target: "琶洲",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "琶洲",
//     target: "万胜围",
//     lineStyle: {
//       normal: {
//         color: "#00A1CC",
//       },
//     },
//   },
//   {
//     source: "高增",
//     target: "清塘",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "清塘",
//     target: "清布",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "清布",
//     target: "莲塘",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "莲塘",
//     target: "马鞍山公园",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "马鞍山公园",
//     target: "花都广场",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "花都广场",
//     target: "花果山公园",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "花果山公园",
//     target: "花城路",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "花城路",
//     target: "广州北站",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "广州北站",
//     target: "花都汽车城",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "花都汽车城",
//     target: "飞鹅岭",
//     lineStyle: {
//       normal: {
//         color: "#99c857",
//       },
//     },
//   },
//   {
//     source: "鱼珠",
//     target: "裕丰围",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "裕丰围",
//     target: "双岗",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "双岗",
//     target: "南海神庙",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "南海神庙",
//     target: "夏园",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "夏园",
//     target: "南岗",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "南岗",
//     target: "沙村",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "沙村",
//     target: "白江",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "白江",
//     target: "新塘",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "新塘",
//     target: "官湖",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "官湖",
//     target: "新沙",
//     lineStyle: {
//       normal: {
//         color: "#a7b942",
//       },
//     },
//   },
//   {
//     source: "嘉禾望岗",
//     target: "白云东平",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "白云东平",
//     target: "夏良",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "夏良",
//     target: "太和",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "太和",
//     target: "竹料",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "竹料",
//     target: "钟落潭",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "钟落潭",
//     target: "马沥",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "马沥",
//     target: "新和",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "新和",
//     target: "太平",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "太平",
//     target: "神岗",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "神岗",
//     target: "赤草",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "赤草",
//     target: "从化客运站",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "从化客运站",
//     target: "东风",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "员村",
//     target: "天河公园",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "天河公园",
//     target: "棠东",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "棠东",
//     target: "黄村",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "黄村",
//     target: "大观南路",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "大观南路",
//     target: "天河智慧城",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "天河智慧城",
//     target: "神舟路",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "神舟路",
//     target: "科学城",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "科学城",
//     target: "苏元",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "苏元",
//     target: "水西",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "水西",
//     target: "长平",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "长平",
//     target: "金坑",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "金坑",
//     target: "镇龙西",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "镇龙西",
//     target: "镇龙",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "镇龙",
//     target: "中新",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "中新",
//     target: "坑贝",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "坑贝",
//     target: "凤岗",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "凤岗",
//     target: "朱村",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "朱村",
//     target: "山田",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "山田",
//     target: "钟岗",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "钟岗",
//     target: "增城广场",
//     lineStyle: {
//       normal: {
//         color: "#203082",
//       },
//     },
//   },
//   {
//     source: "沥滘",
//     target: "南洲",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "南洲",
//     target: "石溪",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "石溪",
//     target: "燕岗",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "燕岗",
//     target: "沙园",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "沙园",
//     target: "沙涌",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "沙涌",
//     target: "鹤洞",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "鹤洞",
//     target: "西塱",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "西塱",
//     target: "菊树",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "菊树",
//     target: "龙溪",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "龙溪",
//     target: "金融高新区",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "金融高新区",
//     target: "千灯湖",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "千灯湖",
//     target: "礌岗",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "礌岗",
//     target: "南桂路",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "南桂路",
//     target: "桂城",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "桂城",
//     target: "朝安",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "朝安",
//     target: "普君北路",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "普君北路",
//     target: "祖庙",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "祖庙",
//     target: "同济路",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "同济路",
//     target: "季华园",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "季华园",
//     target: "魁奇路",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "魁奇路",
//     target: "澜石",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "澜石",
//     target: "世纪莲",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "世纪莲",
//     target: "东平",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "东平",
//     target: "新城东",
//     lineStyle: {
//       normal: {
//         color: "#c9e887",
//       },
//     },
//   },
//   {
//     source: "林和西",
//     target: "体育中心南",
//     lineStyle: {
//       normal: {
//         color: "#00a1cb",
//       },
//     },
//   },
//   {
//     source: "体育中心南",
//     target: "天河南",
//     lineStyle: {
//       normal: {
//         color: "#00a1cb",
//       },
//     },
//   },
//   {
//     source: "天河南",
//     target: "黄埔大道",
//     lineStyle: {
//       normal: {
//         color: "#00a1cb",
//       },
//     },
//   },
//   {
//     source: "黄埔大道",
//     target: "妇儿中心",
//     lineStyle: {
//       normal: {
//         color: "#00a1cb",
//       },
//     },
//   },
//   {
//     source: "妇儿中心",
//     target: "花城大道",
//     lineStyle: {
//       normal: {
//         color: "#00a1cb",
//       },
//     },
//   },
//   {
//     source: "花城大道",
//     target: "大剧院",
//     lineStyle: {
//       normal: {
//         color: "#00a1cb",
//       },
//     },
//   },
//   {
//     source: "大剧院",
//     target: "海心沙",
//     lineStyle: {
//       normal: {
//         color: "#00a1cb",
//       },
//     },
//   },
//   {
//     source: "海心沙",
//     target: "广州塔",
//     lineStyle: {
//       normal: {
//         color: "#00a1cb",
//       },
//     },
//   },
//   {
//     source: "新和",
//     target: "红卫",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "红卫",
//     target: "新南",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "新南",
//     target: "枫下",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "枫下",
//     target: "知识城",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "知识城",
//     target: "何棠下",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "何棠下",
//     target: "旺村",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "旺村",
//     target: "汤村",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "汤村",
//     target: "镇龙北",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
//   {
//     source: "镇龙北",
//     target: "镇龙",
//     lineStyle: {
//       normal: {
//         color: "#721920",
//       },
//     },
//   },
// ]