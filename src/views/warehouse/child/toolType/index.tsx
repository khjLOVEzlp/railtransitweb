...
export const ToolType = () => {
  ...
  const { menu } = useAuth()
  const menuList = menu.find((item: { [item: string]: unknown }) => item.name === "库存管理").childMenu.find((item: { [item: string]: unknown }) => item.name === "仓库管理").childMenu
          ...
          <Table columns={
            [
              ...
              },
              {
                ...
                render: (item: any) => <>
                  {
                    isButton(menuList, "查看库存") && <Button type="link" onClick={() => {
                      setName(item.name)
                      startTool(item.id)
